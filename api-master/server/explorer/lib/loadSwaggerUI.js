...
const accesstokentxtboxid = '#input_accessToken';
const accesstokendisplaycls = '.accessTokenDisplay';
var accessToken;
...
function onInputChange(e) {
  ...
  if (accessToken !== key) {
    $(accesstokendisplaycls).text('Token changed; submit to confirm.');
  } else {
    $(accesstokendisplaycls).text('Token Set.');
  }
}
...
function loadSwaggerUi(config) {
  ...
  window.swaggerUi = new SwaggerUi({
    ...
    onComplete: function (swaggerApi,swaggerUi) {
      ...
      if (window.localStorage) {
        var localStorage = window.localStorage.getItem(lsKey);
        if (localStorage) {
          $(accesstokentxtboxid).val(localStorage).submit();
        }
      }
    },
  ...
  $(accesstokentxtboxid).keyup(onInputChange);
...
function setAccessToken(e) {
  ...
  e.preventDefault();
  var key = $(accesstokentxtboxid)[0].value;
  log('key: ' + key);
  if (key && key.trim() !== '') {
    ...
    accessToken = key;
    $(accesstokendisplaycls).text('Token Set.').addClass('set');
    $(accesstokendisplaycls).attr('data-tooltip','Current Token: ' + key);
    ...
    $(accesstokendisplaycls).text('Token Not Set.').removeClass('set');
    $(accesstokendisplaycls).removeAttr('data-tooltip');
    if (window.swaggerUi) {
