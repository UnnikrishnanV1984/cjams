...
(function(window, document, undefined) {
    ...
    window.Feedback = function(options) {
        ...
        options.messageSuccess = options.messageSuccess || 'Thank you for your valuable feedback.';
        ...
        this.getPersonUrl = options.getPersonUrl;
        ...
        var button,
            ...
            returnMethods = {
                ...
                open: function() {
                    ...
                    modal.className = 'feedback-modal modal-dialog modal-lg';
                ...
                send: function(adapter) {
                    ...
                    adapter.send(data, function(success, response) {
                        emptyElements(modalBody);
                        ...
                        if (success === true) {
                            modalBody.appendChild(document.createTextNode('Thank you for your valuable feedback - Ticket no for this feedback is' + response.supportno));
                        } else {
                            modalBody.appendChild(document.createTextNode(responseText.error.message));
                        }
                    });
                }
            };
    ...
    window.Feedback.Form.prototype.render = function() {
        ...
        var caseId = 'Dashboard';
        var cjamsPid = 'Dashboard';
        if (appUrl.indexOf('/case-worker/') !== -1) {
            caseId = appUrl.split('/')[7];
            const Http = new XMLHttpRequest();
            Http.open('GET', getPersonUrl + '?filter={"count":-1,"page":1,"limit":20,"where":{"intakeserviceid":"' + appUrl.split('/')[6] + '"}}');
            Http.setRequestHeader('access_token', JSON.parse(window.sessionStorage.getItem('token'))['id']);
            Http.send();
            Http.onreadystatechange = e => {
                if (Http.readyState === 4 && Http.status === 200) {
                    if (JSON.parse(Http.responseText).data) {
                        const reportedChild = JSON.parse(Http.responseText).data.filter(child => child.rolename === 'RC')[0];
                        if (reportedChild) {
                            cjamsPid = reportedChild.cjamspid;
                            this.elements[5].element.value = cjamsPid;
                        }
                    }
                }
            };
        } else if (appUrl.indexOf('/my-newintake/') !== -1) {
            caseId = JSON.parse(window.sessionStorage.getItem('intake'))['number'];
            cjamsPid = 'Intake';
        }
        var session = JSON.parse(window.sessionStorage.getItem('token'));
        var now = new Date();
        ...
        prePopulateFeedback = {
            displayname: session.user ? session.user.userprofile.displayname : null,
            cjamspid: session.user ? session.user.userprofile.cjamspid : null,
            frommailid: session.user ? session.user.userprofile.email : null,
            ldssregion: '',
            supportlogdate: localDatetime,
            officelocation: '',
            clientid: cjamsPid,
            subject: '',
            notes: '',
            userrole: session.role ? session.role.description : null,
            caseid: caseId
        };
        var i = 0,
        ...
        for (; i < len; i++) {
            ...
            switch (item.type) {
                ...
                case 'input':
                    ...
                    item.element.setAttribute('name', item.name);
                    if (item.disabled) {
                        item.element.setAttribute('disabled', item.disabled);
                    }
                    item.element.setAttribute('class', item.class);
                    item.element.setAttribute('type', 'text');
                    if (prePopulateFeedback.hasOwnProperty(item.name)) {
                        item.element.value = prePopulateFeedback[item.name];
                    }
                ...
                case 'select':
                    ...
                    item.element.setAttribute('name', item.name);
                    item.element.setAttribute('class', item.class);
                    var optionElements = item.value.map(sev => {
                        var optionElement = document.createElement('option');
                        optionElement.setAttribute('value', sev.id);
                        optionElement.setAttribute('label', sev.text);
                        return optionElement;
                    });
                    optionElements.forEach(opt => {
                        item.element.appendChild(opt);
                    });
                    this.dom.appendChild(item.element);
                    break;
            }
    ...
    window.Feedback.Form.prototype.review = function(dom) {
        ...
        //     }
        // }
        if (this.elements.length > 0 && this.elements[len - 1].element.value.length > 0) {
            dom.appendChild(document.createTextNode(this.elements[len - 1].element.value));
            dom.appendChild(document.createElement('hr'));
        }
        return dom;
    };
    ...
    window.Feedback.Screenshot = function(options) {
        ...
        this.options.blackoutClass = this.options.blackoutClass || 'feedback-blackedout feedback-blackout-element';
        this.options.highlightClass = this.options.highlightClass || 'feedback-highlighted feedback-highlight-element';
    ...
    window.Feedback.Screenshot.prototype.close = function() {
        ...
        removeElements(document.querySelectorAll('.feedback-highlighted, .feedback-highlight-element'));
        removeElements(document.querySelectorAll('.feedback-blackedout, .feedback-blackout-element'));
    };
    ...
    window.Feedback.Screenshot.prototype.data = function() {
        ...
        if (this.h2cCanvas !== undefined) {
            ...
            // draw blackouts
            Array.prototype.slice.call(document.querySelectorAll('.feedback-blackedout'), 0).forEach(function(item) {
                var bounds = getBounds(item);
                ctx.fillRect(bounds.left, bounds.top, bounds.width, bounds.height);
                ctx.drawImage(_self.h2cCanvas, 0, 0);
            });

            // draw highlights
            var items = Array.prototype.slice.call(document.querySelectorAll('.feedback-highlighted'), 0);
    ...
    window.Feedback.XHR.prototype.send = function(data, callback) {
        var xhr = this.xhr;
        const feedbackInput = data[0];
        feedbackInput.filedata = data[1];
        ...
        // xhr.setR
        xhr.setRequestHeader('access_token', JSON.parse(window.sessionStorage.getItem('token'))['id']);
        xhr.send(window.JSON.stringify(feedbackInput));
    };
})(window, document);
