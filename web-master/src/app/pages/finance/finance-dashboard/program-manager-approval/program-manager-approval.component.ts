...
@Component({
    selector: 'program-manager-approval',
    templateUrl: './program-manager-approval.component.html',
    styleUrls: ['./program-manager-approval.component.scss'],
    standalone: false
})
export class ProgramManagerApprovalComponent implements OnInit {
  payableApproval!: any[];
  ...
  statusProgramManagerDropDown(status: any) {
    this.programManagerStatus = status;
    this.getPayableApproval();
  }
  ...
  getPayableApproval(query: any = {}) {
    ...
    this._commonService.getPagedArrayList(
      new PaginationRequest({
        where: {approveltype: 'pmanager', 
        status: this.programManagerStatus,
        roletypekey: 'LDSSPM',
        assigned_pa: true,
 ...
 listUser(assigned: string) {
  ...
  reDirectToProgramManager(fundingData: any) {
    this._router.navigate(['/pages/finance/finance-accountsPayable/approval/director']);
    this._dataStoreService.setData(PurchaseAuthorizationParams.ProgManagerParms, fundingData);
  }
  ...
  callReDirectToProgramManagerMethod(event: any) {
    const data  = JSON.parse(event);
    this.reDirectToProgramManager(data)
  }
