...
updateLoad() {
  ...
  private personattachment(personid:any) {
    ...
    this.personattachmentGridApiFn(inputreq);
  }

  private personattachmentGridApiFn(inputreq: { personid: any; intakenumber: any; servicerequestid: any; servicecaseid: any; adoptioncaseid: any; objecttypekey: string; category: any; subcategory: any; worker: any; title: any; sortcolumn: string; sortby: string; actualdocumentdate: any; }) {
    this._dropDownService
      .getPagedArrayList(
        ...
        Titile4eUrlConfig.EndPoint.Attachment.AttachmentGridUrl + '?filter').subscribe((response: any) => {
          if (response && Array.isArray(response) && response.length) {
            this.personattachmentGridResponseFn(response);
          } else {
            this.userfilteredAttachmentGrid = [];
          }
        });
  ...
  private personattachmentGridResponseFn(response: any[]) {
    ...
    if (result) {
      ...
      this.userfilteredAttachmentGrid.forEach((attach) => {
        attach.uplodeddate = moment(attach.insertedon).format('MM/DD/YYYY hh:mm A');
        attach.actualdocumentdate = (attach.actualdocumentdate) ? moment(attach.actualdocumentdate).format('MM/DD/YYYY') : '';
      });
