...
export class ImageRecordComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        (<any>$('#upload-image')).modal('show'); // NOSONAR
    }
    ...
    saveImage() {
        ...
        this._uploadService
            .upload({
                url: AppConfig.baseUrl + '/' + NewUrlConfig.EndPoint.DSDSAction.Attachment.UploadAttachmentUrl,
    ...
    modalDismiss() {
        (<any>$('#upload-image')).modal('hide'); // NOSONAR
    }
