...
import { CommonHttpService, GenericService, DataStoreService } from '../../../@core/services';
...
@Component({
    ...
    imports:[SortTableModule,MatSortModule,MatFormFieldModule,MatInputModule,MatSelectModule,MatDatepickerModule,ReactiveFormsModule,QuillModule,FormsModule,PaginationModule,CommonModule],
    standalone: true
})
export class DashAssignCaseComponent implements OnInit {
  ...
  getPreIntake: PreIntakeAssign = new PreIntakeAssign;
  showBroadCostMessage: BroadCostMessage = new BroadCostMessage;
  ...
  getRoutingUser(modal: AssignedCase) {
    ...
    if (casecondtion) {
      this.casecondtioncourtordersdescription = casecondtion.casecondtioncourtorders ? casecondtion.casecondtioncourtorders.map((courtOrder: { description: any; }) => courtOrder.description) : [];
  ...
  closePopup() {
    $('#intake-caseassign').modal('hide');
    $('#assign-preintake').modal('hide');
    $('#reopen-intake').modal('hide');
    $('#narrative').modal('hide');
  }
