...
@Component({
    ...
    selector: 'iv-e-cases',
    templateUrl: './iv-e-cases.component.html',
    styleUrls: ['./iv-e-cases.component.scss'],
...
export class IvECasesComponent implements OnInit {
  ...
  cases = [
    {
        'CLIENTID': '100000075',
        'FULLNAME': 'Albie Ellis',
        'REMOVALDATE': '05/04/2018',
        'JURISDICTION': 'ELK RDG',
        'AGENCY': 'DJS',
        'ELIGIBILITYSTATUS': 'Initial Determination',
        'TYPE': 'Foster-Care',
        'ASSIGNDATE': '12/11/2018',
        'STATUS' : 'Pending',
        'SPECIALISTNAME': 'LYDIE QUEBE'
    },
    {
        'CLIENTID': '3195345',
        'FULLNAME': 'Doee Jon',
  ...
  ngOnInit() {
    ...
    this.appealForm
      .get('dispositioncode')?.valueChanges.subscribe(result => {
        if (result === 'ScreenOUT') {
  ...
  searchFoster(value: string) {

    if (value === '100000075') {
        this.router.navigate(['/pages/title4e/foster-care/100000075']);
    }
    if (value === '3195345') {
        this.router.navigate(['/pages/title4e/guardianship/3195345']);
    }
}
  ...
  listUser(assigned: string) {
    ...
    if (assigned === 'TOBEASSIGNED') {
      this.getUsersList = this.returnIntakeUserFn(this.getUsersList);
  ...
  getAssignIntakeUser(modal: PreIntakeAssign) {
    ...
    this._commonService
      ...
      .subscribe(result => {
        ...
        this.getIntakeUsers = this.returnIntakeUserFn(this.getIntakeUsers);
      });
  }
  private returnIntakeUserFn(user: any): RoutingUser[] {
    return user.filter((res: any) => {
