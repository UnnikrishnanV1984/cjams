...
import { GeneralConfig, MasterCatelogConfig, PhoneType } from '../../_entities/general.data.models';
...
export class PhoneTypeComponent implements OnInit {
    ...
    phoneTypeConfig: MasterCatelogConfig;
    paginationInfo: PaginationInfo = new PaginationInfo();
    formGroup: FormGroup;
    dynamicObject: DynamicObject = {};
    phoneTypeData = new GeneralConfig();
    private searchTermStream$ = new Subject<DynamicObject>();
    private pageStream$ = new Subject<number>();

    constructor(
        private formBuilder: FormBuilder,
        private _service: GenericService<PhoneType>,
        private _alertService: AlertService
    ) {
        this._service.endpointUrl =
            AdminUrlConfig.EndPoint.General.PersonPhoneTypeUrl;
        ...
        this.phoneTypeConfig = new MasterCatelogConfig({
            BlockTitle: 'Add Phone Type',
            LabelNameTitle: 'Person Phone Type',
            LabelDescriptionTitle: 'Type Description',
            TableHeaderName: 'Person Phone Type',
    ...
    ngOnInit() {
        this.paginationInfo.sortBy = 'personphonetypekey asc';
        this.getPage();
    }
    ...
    getPage() {
        ...
        const source = merge(searchSource,pageSource).pipe(
            ...
            mergeMap((params: { search: DynamicObject; page: number }) => {
                return this._service
                    ...
                    map(result => {
                        return {
                            data: result.data.map(model => new PhoneType(model)),
                            count: result.count,
        ...
        this.phoneTypes$ = source.pipe(pluck('data'));
    ...
    saveItem(modelData: any) {
        ...
        if (modelData.personphonetypekey) {
            this._service
                .update(modelData.personphonetypekey, modelData)
                .subscribe((response: any) => {
                        if (response) {
                            this._alertService.success(
                                'Phone type saved successfully.'
                            );
            ...
            modelData.personphonetypekey = modelData.name;
            this._service.create(modelData).subscribe((response: any) => {
                    if (response.personphonetypekey) {
                        this._alertService.success(
                            'Phone type  saved successfully.'
                        );
                        ...
                        this.formGroup.patchValue({
                            nameSearch: this.phoneTypeData.name
                        });
                    }
                }, (_error: any) => {
                    this._alertService.error(GLOBAL_MESSAGES.ERROR_MESSAGE);
    ...
    deleteItem(modelData: any) {
        if(modelData.personphonetypekey) {
            this._service.remove(modelData.personphonetypekey).subscribe(
                response => {
                    if (response) {
                        this._alertService.success(
                            'Phone type deleted successfully.'
                        );
                        this.pageStream$.next(this.paginationInfo.pageNumber);
                    }
                },
    ...
    onSort($event: ColumnSortedEvent) {
        if ($event.sortColumn === 'name') {
            this.paginationInfo.sortBy =
                'personphonetypekey ' + $event.sortDirection;
    ...
    onSearch(event: any) {
        if (event.field === 'name') {
            this.dynamicObject['personphonetypekey'] = { like: '%25' + event.value + '%25' };
            if (!event.value) {
                delete this.dynamicObject['personphonetypekey'];
            }
