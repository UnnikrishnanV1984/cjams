...
import { EntityType, GeneralConfig, MasterCatelogConfig } from '../../_entities/general.data.models';
...
export class EntityTypeComponent implements OnInit {
    entityTypeConfig: MasterCatelogConfig;
    ...
    entityTypeData = new GeneralConfig();
    private searchTermStream$ = new Subject<DynamicObject>();
    private pageStream$ = new Subject<number>();
    constructor(
        private formBuilder: FormBuilder,
        private _service: GenericService<EntityType>,
        ...
        this.entityTypeConfig = new MasterCatelogConfig({
            BlockTitle: 'Add Provider Type',
            LabelNameTitle: 'Provider Type',
            LabelDescriptionTitle: 'Type Description',
            TableHeaderName: 'Provider Type',
        ...
        const source = merge(searchSource,pageSource).pipe(
            ...
            mergeMap((params: { search: DynamicObject; page: number }) => {
                return this._service
                    ...
                    map(result => {
                        return {
                            data: result.data.map(model => new EntityType(model)),
                            count: result.count,
                            canDisplayPager: result.count > this.paginationInfo.pageSize
                        };
                    }));
            }),share(),);
        this.entityTypes$ = source.pipe(pluck('data'));
            ...
            this._service.update(modelData.agencytypekey, modelData).subscribe((response: any) => {
                    if (response) {
                        this._alertService.success(
                            'Entity type saved successfully'
                        );
            ...
            this._service.create(modelData).subscribe((response: any) => {
                    if (response.agencytypekey) {
                        ...
                        this.formGroup.patchValue({
                            nameSearch: this.entityTypeData.name
                        });
                    }
                }, (_error: any) => {
                    this._alertService.error(GLOBAL_MESSAGES.ERROR_MESSAGE);
