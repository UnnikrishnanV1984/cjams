-- FUNCTION: public.reassignroutingda(character varying, uuid, character varying, character varying, character varying, json, boolean)

 DROP FUNCTION IF EXISTS reassignroutingda(character varying, uuid, character varying, character varying, character varying, json, boolean);

CREATE OR REPLACE FUNCTION reassignroutingda(appeventcode character varying, intake_service_req_id uuid, fromuserid character varying, v_assignedsecurityid character varying, v_responsibilitytypekey character varying, j_assignfolder json DEFAULT '{}'::json, isnotifyuser boolean DEFAULT true)
...
SELECT  tm.roletypekey  
 ...
 UPDATE routing SET routingstatustypeid =4,activeflag =0  ,
                                 updatedon= v_date
            WHERE objectid  = intake_service_req_id :: character varying;
           
         update  intakeservicerequest set routedusersid=v_assignedsecurityid
         where intakeserviceid=intake_service_req_id;
        
         RAISE NOTICE 'v_assignedsecurityid:%', v_assignedsecurityid; 
 ...
 /* DSDS ACTION TASK GOAL DETAILS(Create action tasks) */
    select createinvestigation INTO s_Actmsg from createinvestigation(
			...
			v_investigationid,
     		...
     		v_roletypecode,
    		v_teamtypekey); 
	 RAISE NOTICE 'createinvestigation :%', s_Actmsg; 		 
...
END IF;   
    ...
    if(isnotifyuser=true) THEN
      SELECT servicerequestnumber into v_intakenumber from intakeservicerequest   
       ...
       SELECT send_notification into v_notifystatus from send_notification(fromuserid, fromuserid,v_assignedsecurityid,
...
if(v_icjprobationcount > 0 and v_fromuserrole='')
   ...
   v_message = 'New case ('|| v_intakenumber||') is assigned by '|| v_createduser ;
        ...
        SELECT send_notification into v_notifystatus from send_notification(v_assignedsecurityid,fromuserid,v_assignedsecurityid,
...
--             cast(intake_service_req_id as character varying));
    ...
    folderopendatetime = v_folderopendatetime , foldernotes = v_foldernotes where intakeserviceid = intake_service_req_id;
