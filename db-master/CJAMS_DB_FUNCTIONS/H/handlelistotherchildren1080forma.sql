DROP FUNCTION IF EXISTS cjams.handlelistotherchildren1080forma(data json, form_id uuid, user_id uuid);
CREATE OR REPLACE FUNCTION cjams.handlelistotherchildren1080forma(data json, form_id uuid, user_id uuid)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
 ...
 IF EXISTS (
 SELECT 1 FROM cjams.listotherchildren1080forma
 ...
 WHERE form1080aid = form_id AND personid = person_id;
 ELSE
 INSERT INTO cjams.listotherchildren1080forma (
