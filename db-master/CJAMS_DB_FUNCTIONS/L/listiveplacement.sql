create index IF NOT EXISTS Xie1_adoptionplanning on adoptionplanning(alternateid);      
--------------
--------------

DROP FUNCTION IF EXISTS listiveplacement(character varying,text[],character varying,character varying,bigint,bigint,character varying,character varying,character varying,character varying,timestamp without time zone,character varying[],character varying,character varying,character varying) ;
DROP function IF EXISTS listiveplacement(character varying,text[],character varying,character varying,bigint,bigint,character varying,character varying,character varying,character varying,timestamp without time zone,character varying,character) ;
CREATE OR REPLACE FUNCTION cjams.listiveplacement(userid character varying, v_status text[], v_roleid character varying, v_placementtype character varying, pagenumber bigint, pagesize bigint, fname character varying, lname character varying, clientiddata character varying, eligiblestatus character varying, rdate timestamp without time zone, todate timestamp without time zone, county character varying[], v_sortorder character varying, filtertype character varying DEFAULT NULL::character varying, assignedspecialist character varying DEFAULT NULL::character varying)
...
---------------------------------------------------------------------------------------
-- 05-27 - CDM-22492 - Veera ACA Cases missing on the dashboard
-- 10-21 - CDM-25798 Veera ACA duplicate cases for IVE specialist
-- CDM-27959 - 01-18-2023 - To get Incomplete Fostercare status
-- CIDM-6805 - Query Optimization
-- CIDM-7298 - Veera Adoption issue fixes for IVE adoption cases 06-13
-- CIDM-7810 - Query Optimization
-- CIDM-8364 - Sundeep kiran Adoption case issue fix for adoption case navigation
-- CDM-41198 - Veera Nadimpalli TO get incomplote cases on dashboard for IVE Specialist - 08/27/2024
-- CDM-42213 - Parshal Chitrakar FosterCare link missing: adding activeflag condition on fetching removalid for adoption case
-- CIDM- 9958 - Veera To get GAP Subsidy ID
-- CIDM-10835 - County Multi select - 11/10/2025
-- CIDM-11127 - IVE Slowness issue fix - 02-11-2026
-- CDM-44770 - IVE Supervisor dashboard Assigned to (fullname_val) fix - 04-02-2026
...
FROM sorted_query pr;
   ...
   RETURN QUERY 
        WITH base_query AS (
            SELECT 
                pl.placementid AS ref_placementid, pl.personid, tce.eligibility_id, isrcr.removalid, isrcr.servicecaseid,
                p.cjamspid, p.lastname, p.firstname, p.middlename, p.dob,
                isrcr.removaldate AS sort_date, isrcr.insertedon AS sort_update,pl.startdatetime::date,
                sc.servicecasenumber::BIGINT as casenumber_val,
                ROW_NUMBER() OVER (PARTITION BY p.cjamspid, sc.servicecaseid ORDER BY pl.alternateid DESC) as base_rownum
            FROM tb_client_eligibility tce
            INNER JOIN person p ON p.cjamspid = tce.client_id AND p.activeflag = 1
            INNER JOIN intakeservreqchildremoval isrcr ON isrcr.removalid = tce.removal_id 
                AND isrcr.activeflag = 1 AND isrcr.exitdate IS NULL
        ...
        filtered_query AS (
            SELECT b.*,
                (SELECT up.fullname::varchar FROM routing r JOIN userprofile up ON up.securityusersid = r.tosecurityusersid
                 JOIN placement plt1 ON r.objectid::varchar = plt1.placementid::varchar
                 WHERE r.activeflag=1 AND r.eventcode='PLTR' AND r.routingstatustypeid::text = '70' AND r.toroleid IN ('IVESP','IVEEA') AND plt1.personid = b.personid
                  AND plt1.activeflag = 1
                 and  (plt1.startdatetime >= b.sort_date OR (plt1.enddatetime IS NULL AND plt1.startdatetime <= b.sort_date))
                 ORDER BY r.insertedon DESC LIMIT 1) as fullname_val,
                (SELECT c.countyname FROM caseassignment ca JOIN county c ON c.countyid = ca.toldssid AND c.activeflag = 1
                 WHERE ca.objectid = b.servicecaseid AND ca.enddate IS NULL ORDER BY ca.insertedon DESC LIMIT 1)::character varying as childjurisdiction_val,
                (SELECT tpv.description_tx::text FROM tb_eligibility_period tep 
                 JOIN tb_picklist_values tpv ON trim(tpv.picklist_value_cd) = trim(tep.status_cd) AND tpv.picklist_type_id = 262
                 WHERE tep.eligibility_id = b.eligibility_id AND tep.delete_sw = 'N' ORDER BY tep.update_ts DESC LIMIT 1) as fostercareeligibilitystatus_val
            FROM base_query b
            WHERE b.base_rownum = 1
        ),
        sorted_query AS (
            SELECT *, COUNT(1) OVER() as total_count
            FROM filtered_query
            WHERE (county IS NULL OR childjurisdiction_val = ANY(county))
              AND (CASE WHEN lower(filtertype) = 'unassigned' THEN fullname_val IS NULL 
                        ...
                        WHEN lower(filtertype) = 'completed' THEN fostercareeligibilitystatus_val IS NOT NULL AND fostercareeligibilitystatus_val NOT IN ('Pending','Incomplete') ELSE TRUE END)
              AND (CASE WHEN assignedspecialist IS NOT NULL THEN lower(fullname_val) = lower(assignedspecialist) ELSE TRUE END)
              AND (CASE WHEN eligiblestatus = 'Pending' THEN (fostercareeligibilitystatus_val IS NULL OR fostercareeligibilitystatus_val = 'Incomplete') 
                        WHEN eligiblestatus IS NOT NULL THEN fostercareeligibilitystatus_val::character varying = eligiblestatus ELSE TRUE END)
            ORDER BY 
                (CASE v_sortorder WHEN 'asc' THEN sort_date END) ASC NULLS LAST,
                (CASE v_sortorder WHEN 'desc' THEN sort_date END) DESC NULLS LAST,
                (CASE WHEN v_sortorder IS NULL THEN sort_update END) DESC NULLS LAST
        ...
        SELECT 
            sr.total_count, NULL::uuid, NULL::uuid, sr.ref_placementid, sr.fullname_val,
            sr.cjamspid, sr.lastname, sr.firstname, sr.sort_date::timestamp, sr.removalid,
            CONCAT(sr.firstname, ' ', sr.middlename, ' ', sr.lastname)::VARCHAR,
            sr.casenumber_val, 'DHS'::character varying, sr.childjurisdiction_val,
            sr.startdatetime,
            sr.dob::date,
            (EXTRACT(year FROM age(sr.sort_date::date, sr.dob::date))::varchar || ' Years ' || EXTRACT(month FROM age(sr.sort_date::date, sr.dob::date))::varchar || ' Months')::varchar,
            sr.servicecaseid,
            (SELECT tep.sqnm_sw::character varying FROM tb_eligibility_period tep WHERE tep.eligibility_id = sr.eligibility_id AND tep.delete_sw = 'N' AND tep.approvalstatus = 'APPROVED' ORDER BY tep.sqnm_sw LIMIT 1),
            sr.fostercareeligibilitystatus_val, NULL::timestamp, sr.base_rownum
