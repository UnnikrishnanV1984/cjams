drop function cjams.getbalanceforancillary(v_client_acc_id bigint);
CREATE OR REPLACE FUNCTION cjams.getbalanceforancillary(v_client_acc_id bigint)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
------------------------------------------------------------------------
-- Revision(s)
-- 03/25/2024 Vineet Tirodkar - Modifications to fix OBLIGATIONS column for -ve obligated amount (CDM-37844)
------------------------------------------------------------------------
begin 
...
SELECT ta.*, to_char(ta.benefit_start_dt  , 'Mon-YY') as benefit_start_dt_month 
	FROM tb_account_transaction ta  
WHERE ta.client_account_id = v_client_acc_id ;

create temp table tmp_client_account  as
select client_account_id ,delete_sw, client_id,status_cd 
	from tb_client_account  
WHERE client_account_id = v_client_acc_id ;

return (
		SELECT 
			   SUM(TAB1.MONTHY_BAL_AVAIL_FOR_ANC)  AS BALANCE_AVAIL_FOR_ANCILLARY
		FROM (
		SELECT TAB.*
		,
		...
		select 
		   ...
		   SUM(
					(CASE WHEN TR.TRANSACTION_TYPE_CD = '589' AND TR.CREDIT_DEBIT_SW = 'C' 
						...
						AND TR.TRANSACTION_SOURCE_CD not in ( '587', '586', '585' ) THEN -- CDM-2885
		  ...
		  - 	
		  SUM(
					(CASE WHEN TR.TRANSACTION_TYPE_CD = '588' AND TR.CREDIT_DEBIT_SW = 'D'  -- ADJUSTMENT_APPROVAL_STATUS_CD = '3047' AND  
						AND TR.TRANSACTION_SOURCE_CD = '5472' THEN
							COALESCE(TR.TRANSACTION_AMOUNT_NO,0)
					ELSE
							0
					END )
