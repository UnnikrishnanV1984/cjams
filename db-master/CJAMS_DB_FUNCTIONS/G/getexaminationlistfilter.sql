DROP FUNCTION IF EXISTS cjams.getexaminationlist_filter(filters json, v_lipagenumber bigint, v_lipagesize bigint); --clean up _ ones in all envs
DROP FUNCTION IF EXISTS cjams.getexaminationlistfilter(filters json, v_lipagenumber bigint, v_lipagesize bigint);
CREATE OR REPLACE FUNCTION cjams.getexaminationlistfilter(filters json, v_lipagenumber bigint, v_lipagesize bigint)
...
return query		
	select count(1) over() as totalcount, x.* from (	
        ...
        and case when startDate is not null then Date(pe.appoinmentdate) >= Date(startDate) else true end
        and case when endDate is not null then Date(pe.appoinmentdate) <= Date(endDate) else true end
