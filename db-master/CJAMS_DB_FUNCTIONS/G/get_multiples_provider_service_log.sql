DROP FUNCTION IF EXISTS get_multiples_provider_service_log(uuid);

CREATE OR REPLACE FUNCTION cjams.get_multiples_provider_service_log(casenumber text, v_role_id character varying, clientids text, v_pagenumber integer DEFAULT NULL::integer, v_limit integer DEFAULT NULL::integer)
 RETURNS TABLE(totalcount bigint, service_log_id bigint, provider_service_id bigint, provider_id integer, provider_nm text, service_name character varying, service_id integer, service_type character varying, tax_id numeric, zip numeric, frequency_cd character varying, frequency character varying, duration_cd character varying, duration character varying, agency_program_area_id character varying, agency_program_nm character varying, service_end_reason_cd character varying, service_end_reason character varying, no_service_reason_cd character varying, no_service_reason character varying, actual_start_date date, actual_end_date date, estimated_start_date date, estimated_end_date date, start_time character varying, end_time character varying, referred_date date, courtorder character, notes character varying, outcome character varying, status text, serviceplanactionid uuid, serviceplanid uuid, serviceplanname character varying, serviceplanactionname character varying, current_dt date, ongoingtransflag boolean, authflag boolean)
 LANGUAGE plpgsql
AS $function$
...
-- left join tb_service_purchase_authorization tspa on tspa.service_log_id = tsl.service_log_id
 ...
 left join serviceplanaction spla  on spla.serviceplanactionid =TSL.serviceplanactionid and spla.activeflag=1
  ...
  AND TSL.client_id =  ANY( v_clientids::bigint[]) 
  AND TSL.delete_sw='N'
