DROP FUNCTION IF EXISTS cjams.getuserdetailsbyemail(character varying, character);
DROP FUNCTION IF EXISTS cjams.getuserdetailsbyemail(character varying);


CREATE OR REPLACE FUNCTION cjams.getuserdetailsbyemail(p_email character varying)
RETURNS TABLE (
    firstname character varying,
    lastname character varying,
    middlename character varying,
    username character varying,
    dhsid character varying,
    mobilenumber character varying,
    workphonenumber character varying,
    localdepartmentname character varying,
    supervisorname character varying,
    supervisoremail character varying,
    workeremail character varying,
    county character varying,
    countycode character varying,
    agencycode character varying,
    teamcode character varying,
    teamname character varying,
    openamrole character varying[],
    positioncode character varying,
    positiontitle character varying,
    assupervisoremail character varying,
...
LANGUAGE plpgsql
AS $function$
-- 3/17/2025  Anil Dharni - CIDM-10293- Created procedure to retrieve user details by email address
-- 4/01/2025  Anil Dharni - CIDM-10293- Modified the SP to accept agencycode as the parameter
-- 5/08/2025  Anil Dharni - CIDM-10293- Modified getuserdetailsbyemail to return all the values that are being
                                    -- sent as part of the create payload including openamrole
-- 5/08/2025  Enhancement - CIDM 10293 Added UNION to return all users (both active and inactive)
-- 5/08/2025  Anil Dharni - CIDM 10293 Added Drop statement
-- 6/9/2025 Anil Dharni - CIDM-10293 - Changes to fetch all roles assigned to a user
-- 6/9/2025 Anil Dharni - CIDM-10293 - Fix for openamroles being in response even after user offboarding.
BEGIN
    RETURN QUERY
    SELECT DISTINCT
        u.firstname, 
    ...
    INNER JOIN rolemapping rm
        on mu.id::varchar = rm.principalid and  
        case when u.activeflag = 1 then rm.activeflag = 1
        else rm.id in 
