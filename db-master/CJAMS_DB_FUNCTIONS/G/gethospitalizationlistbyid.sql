drop function if exists gethospitalizationlistbyid(uuid);
CREATE OR REPLACE FUNCTION cjams.gethospitalizationlistbyid(hospitalization_id uuid)
...
-- 10/01 - Veera Nadimpalli - LA User story changes
...
BEGIN 
	...
	return query		
select count(1) over() as totalcount,ph.hospitalizationid, ph.hospitalization_type,(select value_tx from tb_picklist_values where trim(picklist_value_cd)=trim(ph.hospitalization_type) and picklist_type_id=230 limit 1) as hospitalization_typedesc,ph.hospitalization_reason,ph.hospitalization_discharge_recommendation_others, ph.actual_placement_after_discharge_others,(select value_tx from tb_picklist_values where trim(picklist_value_cd)=trim(ph.hospitalization_reason) and picklist_type_id in(310,280) limit 1) as hospitalization_reasondesc,diagnosistx as "Reason_or_diagnosis",
...
ph.hospital_examstartdate as "Hospital_examStartDate",
ph.hospital_evaluatstartdate as "Hospital_evaluatSartDate",
ph.hospital_overstaydate as "Hospital_OverstayDate",
ph.hospital_transferdate as "Hospital_TransferDate",
ph.hospital_inpatientadmissiondate as "Hospital_InpatientAdmissionDate",
ph.hospital_dischargeddate as "Hospital_DischargedDate",
...
where ph.hospitalizationid=hospitalization_id; 
END;
