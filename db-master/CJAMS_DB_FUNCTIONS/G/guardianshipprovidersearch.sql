Drop FUNCTION if exists cjams.guardianshipprovidersearch(searchobj json, v_lipagenumber bigint, v_lipagesize bigint);
CREATE OR REPLACE FUNCTION cjams.guardianshipprovidersearch(searchobj json, v_lipagenumber bigint, v_lipagesize bigint)
...
AS $function$
-- Revision(s)
-- 11/29 ---Umasankar Raavi -- Type cast changes -- CDM-26849     
...
v_pageoffset := v_pagenumber * v_liPageSize;
  ...
  RETURN query
	...
	(select concat_ws(' ',tpa.adr_street_tx,tpa.adr_box_no,tpa.adr_unit_no_tx,tpa.adr_street_nm,tpa.adr_street_suffix_cd,tpa.adr_city_nm,tpa.adr_state_cd,tpa.adr_zip5_no)
    from tb_provider_addresses tpa 
	where   tpa.parent_key_id = TBP.provider_id::character varying
	-- and adr_format_cd in ('P','S') 
		and tpa.delete_sw  = 'N'
		and tpa.adr_type_cd  = '3357'
		and tpa.adr_default_sw  = 'Y'
ORDER BY tpa.adr_start_dt desc
Limit 1)::character varying as address,
	...
	AND TBP.delete_sw = 'N' 
	AND (TBAA.approval_type_cd IN ('4992', '3577', '4990') OR TBPS.service_id = 503)
