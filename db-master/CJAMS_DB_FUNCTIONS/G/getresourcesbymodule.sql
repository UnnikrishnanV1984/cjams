...
DROP FUNCTION IF EXISTS cjams.getresourcesbymodule(v_modulename character varying, v_page integer, v_limit integer);
CREATE OR REPLACE FUNCTION cjams.getresourcesbymodule(v_modulename character varying, v_page integer, v_limit integer)
...
AND	CASE WHEN v_modulename IS NOT NULL THEN rs.resourceid like v_modulename || '%'ELSE true END
