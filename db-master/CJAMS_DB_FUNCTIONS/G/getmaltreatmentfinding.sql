DROP FUNCTION IF EXISTS cjams.getmaltreatmentfinding(v_investigationid uuid);
CREATE OR REPLACE FUNCTION cjams.getmaltreatmentfinding(v_investigationid uuid)
 RETURNS json
 LANGUAGE plpgsql
AS $function$

------------------------------------------------------------------------------------------------------------
-- Function Note: This proc has the expunge proc. If changing to getmaltreatmentfinding, similar updates are also required for getmaltreatmentfinding_expunge
-- Expungement Proc : cjams.getmaltreatmentfinding_expunge
------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------
-- Revision(s)
...
SELECT reporteddate into l_reporteddate FROM intakeservicerequest where intakeserviceid =  l_serviceid;
		 ...
		 select json_agg(allegation) into l_investigation from (
			...
			(select fatalitycommentaudittrail from investigation i where investigationid =v_investigationid and activeflag=1 ORDER BY updatedon DESC LIMIT 1) as commentaudittrail,
              (SELECT json_agg(r)FROM (
			...
			doculaw )::json as law_enforcement_attachment ,
					...
					(SELECT json_agg(ind) FROM (
							...
							(SELECT RS.description
								...
								Inner Join 
									intakeservicerequestactor israa
						    ...
						    JOIN intakeservicerequestactor isra ON isra.intakeservicerequestactorid= iai.intakeservicerequestactorid and isra.intakeserviceid is not null --and isra.intakeservicerequestpersontypekey = 'AM'
							and (isra.activeflag = 1 or (isra.activeflag = 0 and isra.updatedby  = 'EXPUNG'))
                	 ...
                	 (SELECT json_agg(finding) FROM (
							...
							CASE WHEN IFN.finalfinding IS NULL THEN (SELECT iaml.overridefindingtypekey FROM Investigationallegationmaltreators iaml WHERE iaml.investigationallegationid = IFN.investigationallegationid AND iaml.activeflag =1 AND iaml.overrideapprflag =1 order by iaml.updatedon desc LIMIT 1) 
            ...
            INNER JOIN intakeservicerequestactor isra1 on isra1.intakeservicerequestactorid = ima.intakeservicerequestactorid and isra1.activeflag = 1
