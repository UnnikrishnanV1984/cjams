-- FUNCTION: cjams.get_payment_header(json)

DROP FUNCTION cjams.get_payment_header(json);

CREATE OR REPLACE FUNCTION cjams.get_payment_header(
	request json)
    RETURNS TABLE(totalcount bigint, payment_id integer, gross_amount_no numeric, offset_amount_no numeric, payment_start_dt date, payment_end_dt date, payment_type_cd character varying, payment_type_nm character varying, payment_status_nm character varying) 
    LANGUAGE 'plpgsql'
...
RAISE NOTICE 'v_payment_type_cd%', v_payment_type_cd;
...
,payhead.payment_start_dt
,payhead.payment_end_dt
...
AND PS.payment_id=payhead.payment_id limit 1) as payment_status_nm
from tb_payment_header payhead 
INNER join tb_payment_detail paydet on paydet.payment_id=payhead.payment_id 
left join person client on client.cjamspid=paydet.client_id 
...
WHERE payhead.delete_sw='N' and payhead.payment_type_cd not in  ('5989','4','21','22')  and  payhead.manual_sw != 'Y'
...
group by payhead.payment_id,payhead.gross_amount_no,payhead.offset_amount_no,payhead.payment_start_dt,payhead.payment_end_dt,
payhead.payment_type_cd,payhead.manual_sw
