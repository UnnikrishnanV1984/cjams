...
DROP FUNCTION IF EXISTS getpsychotropiccalculateavgtime(
...
CREATE OR REPLACE FUNCTION getpsychotropiccalculateavgtime(
    v_medicationname character varying DEFAULT NULL::character varying,
    v_prescribername character varying DEFAULT NULL::character varying,
    v_dateprescribed date DEFAULT NULL,
    v_submissiondate date DEFAULT NULL,
    filterdatetype character varying DEFAULT NULL::character varying,
    v_clientname character varying DEFAULT NULL::character varying,
    v_age int DEFAULT NULL,
    v_countyid character varying DEFAULT NULL,
    startdate date DEFAULT NULL,
    enddate date DEFAULT NULL,
    v_teamid character varying DEFAULT NULL,
    v_caseworkid character varying DEFAULT NULL
) RETURNS JSON AS $$
--------------------------------------------------------------------------------------------------
-- 03/11/2023 prasanna sai kommineni - CIDM-10188 B-215263 : Psychotropic Prescription Review Report Dashboard
...
BEGIN
    ...
    SELECT
       COALESCE(
        FLOOR(SUM(EXTRACT(epoch FROM (CASE WHEN r.insertedon = r.updatedon THEN now() ELSE r.updatedon END) - r.insertedon)) 
             / COUNT(DISTINCT psm.psychotropicid) / 86400), 0
    )::TEXT || ' D, ' ||
 COALESCE(
        FLOOR(MOD(SUM(EXTRACT(epoch FROM (CASE WHEN r.insertedon = r.updatedon THEN now() ELSE r.updatedon END) - r.insertedon))::integer 
                 / COUNT(DISTINCT psm.psychotropicid), 86400) / 3600), 0
    )::TEXT || ' H, ' ||
 COALESCE(
        FLOOR(MOD(SUM(EXTRACT(epoch FROM (CASE WHEN r.insertedon = r.updatedon THEN now() ELSE r.updatedon END) - r.insertedon))::integer
