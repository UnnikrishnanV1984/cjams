CREATE OR REPLACE FUNCTION cjams.getpaymenthistorybyclientid(searchobj json, v_lipagesize bigint, v_lipagenumber bigint)
...
-- 10/12/2023 Vineet Tirodkar - Modifications to fix duplicate payment detail Id display issue (CDM-34371)
------------------------------------------------------------------------------------------------------------	
DECLARE 
	v_clientid bigint;
	v_pagenumber int;
...
return query
	select count(1) over() as totalcount, 
		...
		x.paymentdetail,
		x.caseworker,
		x.fiscal_category_cd
	from 
		(select payhead.provider_id,
			(case when (trim(payhead.payment_type_cd) = '5989') then 
				payhead.payee_nm
			else 
			...
			where TRIM(PICKLIST_VALUE_CD)= payhead.payment_type_cd
			...
			coalesce(payhead.offset_amount_no,0.00) as offset_amount_no, 
			(payhead.gross_amount_no + coalesce(payhead.offset_amount_no,0.00)) as netamount,
			(select jsonb_agg(x) 
				from ( select sum(paydet.final_amount_no) 
					   ...
					   where paydet.payment_id = payhead.payment_id 
							and paydet.client_id = v_clientid
					  ) 
			as x) as paymentdetail,
			/*
			(select jsonb_agg(x) from ( 
				select sum(paydet.final_amount_no), * 
					from (select  sum(paydet.final_amount_no) 
							from tb_payment_detail paydet 
								join tb_payment_header tphh on tphh.payment_id = paydet.payment_id
						  where paydet.payment_id = payhead.payment_id and paydet.client_id = v_clientid
						 ) as y)
		    as x) as paymentdetail,
			*/
			(select jsonb_agg(a) 
				from (select  count(1) over(), * 
				from (select max(d.case_id) as case_id 
	...
	where paydet.client_id = v_clientid and (paymenttype is null or payhead.payment_type_cd=paymenttype)
		...
		and trim(payhead.payment_type_cd) not in ('21','22')
	...
	*/	
	order by (CASE sortorder
				...
				THEN
					 CASE sortcolumn
						WHEN 'paymentDate' THEN x.payment_dt::character varying || ',' || x.payment_id::character varying 
				...
				ELSE
					x.payment_dt::character varying || ',' || x.payment_id::character varying
					 -- cast(x.payment_dt  as character varying)
				END
		END) ASC NULLS LAST,
		(CASE sortorder
