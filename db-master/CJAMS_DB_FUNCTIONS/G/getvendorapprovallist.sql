DROP FUNCTION IF EXISTS cjams.getvendorapprovallist(json);
CREATE OR REPLACE FUNCTION cjams.getvendorapprovallist(searchobj json)
 RETURNS TABLE(displayname character varying, email character varying, title character varying, securityuserid character varying, teamtypekey character varying, usertypekey character varying)
 LANGUAGE plpgsql
AS $function$
...
SELECT routingstatustypekey,sourcerolekey,targetrolekey,
	...
	for v_routing_rec in  
	           SELECT 	tma.securityusersid
				...
				WHERE  	tm.roletypekey='FNSFS' AND t.activeflag=1
		   ...
		   RETURN QUERY	
			select up.displayname,up.email,up.title,up.securityusersid as securityuserid,up.teamtypekey,up.usertypekey from userprofile up 
			where up.securityusersid = v_routing_rec.securityusersid  and up.activeflag = 1;
