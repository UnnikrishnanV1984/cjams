...
BEGIN
 ...
 return query 
select count(1) over() as totalcount,* from (select distinct on (r.objectid) r.objectid,r.eventcode,r.fromsecurityusersid,r.tosecurityusersid,r.teamid,r.routingstatustypeid,r.isreviewrequest,
r.remarks,r.routeddescription,r.servicerequestnumber,r.objecttypekey,tpa.jurisdiction,tpa.applicant_id,tpa.prgram,tpa.application_status,tpa.organization_first_nm,tpa.organization_tax_id,
...
tpa.co_applicant_employer_nm,tpa.co_applicant_phone_nm,r.insertedon,ct.countyname
from routing r join tb_public_provider_applicant tpa on r.objectid = tpa.applicant_id 
inner join county ct on ct.countyid = (tpa.jurisdiction)::uuid
where
--r.tosecurityusersid=v_tosecurityusersid and 
r.activeflag = 1 and r.eventcode != 'PRWS'
and (v_status is null or tpa.application_status = ANY (v_rstatus))
and (v_jurisdiciton is null or tpa.jurisdiction = v_jurisdiciton))as x
