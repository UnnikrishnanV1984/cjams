...
---07/20/2022 Shamili -- Adding fileds for AFCARS B-130716
    --new attributes list
...
drop function if exists  getbehavioursubstance(uuid,bigint,bigint);
CREATE OR REPLACE FUNCTION cjams.getbehavioursubstance(person_id uuid, v_lipagenumber bigint, v_lipagesize bigint)
...
from documentproperties dp where dp.additionalobjectid = pbh.personbehavioralhealthid::varchar and dp.additionalobjecttype = 'personbehavioralhealth' and dp.activeflag = 1
)docs) as uploadpath, 
pbh.reportname,
pbh.email,
pbh.evaluationby,
