...
DROP FUNCTION IF EXISTS cjams.getpsychotropicmedicationreportbusinessdays(startdate date, enddate date,v_medicationname character varying, v_prescribername character varying ,v_dateprescribed date ,v_submissiondate date
...
CREATE OR REPLACE FUNCTION cjams.getpsychotropicmedicationreportbusinessdays(startdate date DEFAULT NULL, enddate date DEFAULT NULL, 
...
v_pagenumber  :=  (page-1)*10; 
   ...
   SELECT json_agg(a) INTO v_resultcount FROM (
    ...
    SELECT 
       ...
       SELECT 
        ...
        -- (SELECT (EXTRACT(day FROM (r1.insertedon - psm.insertedon)) || ' D, ' ||
        --          EXTRACT(hour FROM (r1.insertedon - psm.insertedon)) || ' H, ' ||
        --          EXTRACT(minute FROM (r1.insertedon - psm.insertedon)) || ' M ')
        --  FROM routing r1 
        --  WHERE r1.fromroleid = 'CWCW' 
        --    AND r1.routingstatustypeid IN (908) 
        --    AND r1.objectid = psm.psychotropicid::VARCHAR 
        --  LIMIT 1) AS timetakenbycaseworker,
        (select cjams.getbusinessdaysbetween(max(r1.insertedon)::timestamp with time zone,(CASE WHEN max(r1.insertedon) = max(r1.updatedon) THEN now() ELSE max(r1.updatedon) END)::timestamp with time zone,'psychotropic_business_hour_start','psychotropic_business_hour_end' ) as timetakenbycoordinator
  									 from routing r1 where r1.toroleid='CWCW' and r1.routingstatustypeid IN (904)
  									and r1.objectid =psm.psychotropicid :: VARCHAR   
  									group by r1.objectid) as timetakenbycaseworker,
         (SELECT
           COALESCE(
        FLOOR(SUM(  cjams.getcalculatebusinessdays(r1.insertedon::timestamp,(CASE WHEN r1.insertedon = r1.updatedon THEN now() ELSE r1.updatedon END)::timestamp ,'psychotropic_business_hour_start','psychotropic_business_hour_end'  )) 
              / (8 * 3600)), 0
    )::TEXT || ' D, ' ||
 COALESCE(
        FLOOR(MOD(SUM( cjams.getcalculatebusinessdays(r1.insertedon::timestamp,(CASE WHEN r1.insertedon = r1.updatedon THEN now() ELSE r1.updatedon END)::timestamp  ,'psychotropic_business_hour_start','psychotropic_business_hour_end' )) 
                 , (8 * 3600)) / 3600), 0
    )::TEXT || ' H, ' ||
 COALESCE(
        FLOOR(MOD(SUM( cjams.getcalculatebusinessdays( r1.insertedon::timestamp, (CASE WHEN r1.insertedon = r1.updatedon THEN now() ELSE r1.updatedon END)::timestamp ,'psychotropic_business_hour_start','psychotropic_business_hour_end'  )) 
 ...
 COALESCE(
        FLOOR(MOD(SUM( cjams.getcalculatebusinessdays( r1.insertedon::timestamp, (CASE WHEN r1.insertedon = r1.updatedon THEN now() ELSE r1.updatedon END)::timestamp ,'psychotropic_business_hour_start','psychotropic_business_hour_end'  )) 
  		...
  		(SELECT 
    cjams.getbusinessdaysbetween(r1.insertedon::timestamp with time zone,
        (CASE 
           WHEN r1.updatedon = r1.insertedon THEN CURRENT_TIMESTAMP 
           ELSE r1.updatedon 
         END)::timestamp with time zone ,'psychotropic_business_hour_start','psychotropic_business_hour_end' 
         
    ) as timeundercommonpool
FROM routing r1 
WHERE r1.fromroleid = 'CWCW' 
  AND r1.routingstatustypeid IN (908) 
  AND r1.objectid = psm.psychotropicid :: VARCHAR
LIMIT 1
  ...
  (select  cjams.getbusinessdaysbetween( (select r2.insertedon  from routing r2 where r2.eventcode='PSY'  
and r2.objectid =psm.psychotropicid  :: VARCHAR  order by r2.insertedon asc limit 1),
(case when r1.routingstatustypeid IN (16,905) then r1.insertedon else (CASE WHEN r1.insertedon = r1.updatedon THEN now() ELSE r1.updatedon END)end),'psychotropic_business_hour_start','psychotropic_business_hour_end' ) 
