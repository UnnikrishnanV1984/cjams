-- FUNCTION: cjams.get_provider_service_log(text, character varying, bigint, bigint, integer, integer)


DROP FUNCTION if exists cjams.get_provider_service_log(text, character varying, bigint, bigint, integer, integer);
DROP FUNCTION if exists cjams.get_provider_service_log(text, character varying, bigint, bigint, integer, integer, character varying, character varying);

CREATE OR REPLACE FUNCTION cjams.get_provider_service_log(
	casenumber text,
	v_role_id character varying,
	clientid bigint,
	v_service_log_id bigint,
	v_pagenumber integer DEFAULT NULL::integer,
	v_limit integer DEFAULT NULL::integer,
	sortorder character varying default null:: character varying,
	sortcolumn character varying default null:: character varying)
    RETURNS TABLE(totalcount bigint, service_log_id bigint, provider_service_id bigint, provider_id integer, provider_nm text, service_name character varying, service_id integer, service_type character varying, tax_id numeric, zip numeric, frequency_cd character varying, frequency character varying, duration_cd character varying, duration character varying, agency_program_area_id character varying, agency_sub_program_area_id character varying, agency_program_nm character varying, service_end_reason_cd character varying, service_end_reason character varying, no_service_reason_cd character varying, no_service_reason character varying, actual_start_date date, actual_end_date date, estimated_start_date date, estimated_end_date date, start_time character varying, end_time character varying, referred_date date, courtorder character, notes character varying, endReason character varying, endReasonNotes character varying, outcome character varying, status text, serviceplanactionid uuid, serviceplanid uuid, serviceplanname character varying, serviceplanactionname character varying, current_dt date, ongoingtransflag boolean, authflag boolean, servicelogminenddt date) 
    LANGUAGE 'plpgsql'
...
if (v_service_log_id is null) then
RETURN QUERY
...
--			END AS "zip"
			,(select TPA.adr_zip5_no  from  tb_provider_addresses  AS TPA where  TPA.parent_key_id  = TPR.provider_id::varchar and TPA.delete_sw='N' limit 1)
	  ...
	  (select 
	  		CASE  WHEN r.toroleid=v_role_id AND r.routingstatustypeid !=62 
	   		...
	   		END as remarks
	  from routing r 
	  join tb_service_purchase_authorization tspa on (to_char_int(tspa.authorization_id)) = r.objectid and tspa.delete_sw='N'
  ...
  (case when exists (  select r.remarks from tb_service_purchase_authorization tspa
	  inner join routing r on r.objectid=(to_char_int(tspa.authorization_id)) and r.eventcode in ('PCAUTH','PCAUTHR') and  remarks not in ('Denied','Approved') and remarks is not null and remarks != '' and r.activeflag=1
	  where tspa.service_log_id=TSL.service_log_id)
    then true 
    when exists (  select r.remarks from tb_service_purchase_authorization tspa
	  inner join routing r on r.objectid=(to_char_int(tspa.authorization_id)) and r.eventcode in ('PCAUTH','PCAUTHR') and r.remarks  in ('Approved') and r.activeflag=1
  ...
  (case when exists (select * from tb_service_purchase_authorization tspa
	  where tspa.service_log_id=TSL.service_log_id) then true else false end) as authflag,
  (select COALESCE(Max(end_dt),TSL.start_dt) from tb_service_purchase_authorization tspa where tspa.service_log_id=TSL.service_log_id and tspa.delete_sw='N' and 
	  (select count(1) = 0 from routing where objectid in (authorization_id:: character varying) and routingstatustypeid = 62
		and routing.activeflag=1  and eventcode in ('PCAUTH','PCAUTHR'))) as servicelogminenddt
...
-- left join tb_service_purchase_authorization tspa on tspa.service_log_id = tsl.service_log_id
...
order by (case sortorder when 'asc' then 
									case sortcolumn when 'provider_id' then cast(TPR.provider_id as character varying) 
													when 'provider_nm' then cast(
												 CASE WHEN TPR.provider_nm IS NOT NULL and tpr.provider_nm !=''
											        THEN TPR.provider_nm else concat_ws(' ',tpr.provider_first_nm,tpr.provider_middle_nm,tpr.provider_last_nm)      		  
													end as character varying) 
													when 'actual_start_date' then cast(TSL.start_dt as character varying)
													when 'actual_end_date' then cast(TSL.end_dt as character varying)
									else cast(TSR.Service_nm as character varying) 
									end
				END) ASC NULLS LAST,
				(case sortorder when 'desc' then 
									case sortcolumn when 'provider_id' then cast(TPR.provider_id as character varying) 
												 ...
												 CASE WHEN TPR.provider_nm IS NOT NULL and tpr.provider_nm !=''
													...
													when 'actual_start_date' then cast(TSL.start_dt as character varying)
													when 'actual_end_date' then cast(TSL.end_dt as character varying)
									else cast(TSL.start_dt as character varying) 
									end
				END) DESC NULLS LAST,
			 (CASE sortcolumn
				WHEN 'actual_start_date' THEN tsl.service_log_id
				WHEN 'actual_end_date' THEN tsl.service_log_id
			END) ASC
...
order by (case sortorder when 'asc' then 
				...
				END) DESC NULLS LAST,
				(CASE sortcolumn WHEN 'actual_start_date' THEN tsl.service_log_id
					WHEN 'actual_end_date' THEN tsl.service_log_id
				END) ASC
