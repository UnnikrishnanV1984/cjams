DROP FUNCTION IF EXISTS getchild113report(v_providerid integer, v_lipagesize bigint, v_lipagenumber bigint, date_sw character varying , date_from date , date_to date );
CREATE OR REPLACE FUNCTION cjams.getchild113report(v_providerid integer, v_lipagesize bigint, v_lipagenumber bigint, date_sw character varying DEFAULT NULL::character varying, date_from date DEFAULT NULL::date, date_to date DEFAULT NULL::date)
...
concat(client.firstname,' ',client.lastname) as client_nm,
 0 as client_gross_amount,
 0 as receivable_amount,
 0 as client_net_amount,
 0 as check_dt,
 0 as check_no,
 0 as from_date_arg,
0 as to_date_arg,
0 as provider_id_arg,
0 as county_cd_arg,
0 as county_name_arg,
0 as payment_type_arg,
0 as payment_id_arg,
0 as client_id_arg,
0 as case_id_arg,
0 as account_receivable_tx_arg,
...
,(CASE WHEN PROV.PROV_TAX_TYPE_CD = '2518' THEN 
	  ...
	  ELSE 
		LEFT((PROV.TAX_ID_NO::varchar),2) || '-' || SUBSTR((PROV.TAX_ID_NO::varchar),2, LENGTH(PROV.TAX_ID_NO::varchar))
	  END 
	)AS   tax_id
...
,(select distinct up.fullname from servicecase ISR
...
where   isr.activeflag=1  and up.activeflag=1 and  ISR.servicecasenumber=paydet.case_id:: character varying
) as case_worker_nm
...
and 
			(date_sw is null 
		  	...
		  	--		AND to_date( cast(CURRENT_DATE as TEXT), 'YYYY-MM-DD' ) 		
			  when date_sw = 'Y' then
...
payhead.payment_dt,payhead.payment_type_cd,
payhead.check_status_cd,payhead.gross_amount_no,
payhead.offset_amount_no,paydet.case_id,isrt.description,client.firstname,client.lastname,
payhead.payment_start_dt,payhead.payment_end_dt,
payhead.adr_county_cd,paydet.client_id,
paydet.reference_payment_detail_id,paydet.FINAL_FISCAL_CATEGORY_CD,
prov.provider_first_nm,prov.provider_last_nm,
paydet.final_service_id,service_nm,paydet.final_service_start_dt,paydet.final_service_end_dt,
paydet.final_amount_no,paydet.status_cd,
prov.mail_code_tx,prov.tax_id_no,prov.prov_tax_type_cd,prov.provider_nm,local_department,case_worker_nm,payment_status_nm
