DROP FUNCTION If exists cjams.getactivitytaskbyusercount(userid character VARYING);
CREATE OR REPLACE FUNCTION cjams.getactivitytaskbyusercount(userid character VARYING, filterdatetype character varying)
 RETURNS TABLE(count bigint)
 LANGUAGE plpgsql
AS $function$
 -------------------------------------------------------------------------------------------
--Revision(s)
-- 12/27/2023 Palani/Manasa - Query optimization changes(CIDM-8261)
-------------------------------------------------------------------------------------------  
DECLARE
...
where TMA.securityusersid = userid AND (v_DateFrom IS NULL OR date(acttask.duedate) between CAST(v_DateFrom AS DATE) AND CAST(v_DateTo AS DATE))
AND ISR.activeflag = 1 AND ISR.teamtypekey = 'CW' --AND atst.typedescription = 'Open'
and acttask.activitytaskstatustypekey in ('AllegOpen','InvOpen');                                                          
