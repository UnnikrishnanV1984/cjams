DROP FUNCTION IF EXISTS getassignmentstatus(v_adoptioncaseid uuid);

CREATE OR REPLACE FUNCTION cjams.getassignmentstatus(v_adoptioncaseid uuid)
 RETURNS TABLE(responsibilitytypekey character varying, caseassignmentid uuid, casetype character varying, countyname character varying, teamname character varying, startdate timestamp without time zone, enddate timestamp without time zone, statustypekey character varying, remarks character varying, toteamid uuid, toworkeridno character varying, fromworkerdetails json, toworkerdetails json, loadnumber character varying, localdepartment character varying, childinfo json, servicecaseid uuid)
 LANGUAGE plpgsql
AS $function$
...
BEGIN  
  ...
  SELECT 
	   ...
	   CA.toworkeridno,
		...
		(SELECT  json_agg(x)  
		 FROM  (
			...
			WHERE 	CAA.caseassignmentid = CA.caseassignmentid AND CAA.activeflag =1	 )  AS  x  
		)  AS  childinfo,
		CA.objectid as servicecaseid
			
	FROM caseassignment CA 
	INNER JOIN adoptioncase ad ON ad.servicecaseid = ca.objectid AND ad.activeflag =1
	...
	WHERE ad.adoptioncaseid = v_adoptioncaseid ORDER BY COALESCE(CA.enddate:: date,now() + interval '1' day ) desc ; 
END;
