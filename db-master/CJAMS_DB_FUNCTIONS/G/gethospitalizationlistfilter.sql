DROP FUNCTION IF EXISTS cjams.gethospitalizationlist_filter(json,bigint,bigint); --clean up _ ones in all envs
DROP FUNCTION IF EXISTS cjams.gethospitalizationlistfilter(json,bigint,bigint);
CREATE OR REPLACE FUNCTION cjams.gethospitalizationlistfilter(filters json, v_lipagenumber bigint, v_lipagesize bigint)
...
from documentproperties dp where dp.additionalobjectid = ph.hospitalizationid::varchar and dp.additionalobjecttype = 'personhospitalization' and dp.activeflag in (1,3,4,5)
)docs) as uploadpath, 
...
where ph.activeflag in (1 , 2) and ph.personid=person_id 
and case when startDate is not null and ph.hospital_inpatientadmissiondate is not null then Date(ph.hospital_inpatientadmissiondate) >= Date(startDate) else true end
and case when endDate is not null and  ph.hospital_dischargeddate is not null then Date(ph.hospital_dischargeddate) <= Date(endDate) else true end
order by ph.hospital_examstartdate , ph.hospital_inpatientadmissiondate desc
