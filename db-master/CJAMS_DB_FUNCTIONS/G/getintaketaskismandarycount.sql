 CREATE OR REPLACE FUNCTION public.getintaketaskismandarycount(intakserviceid uuid)                                         
  RETURNS TABLE(ismandatorycount bigint)                                                                                    
  LANGUAGE plpgsql                                                                                                          
 AS $function$                                                                                                              
                                                                                                                            
 BEGIN                                                                                                                      
          RETURN QUERY                                                                                                      
        select count(1) as ismandatorycount from investigation IVS                                                          
  ...
  and Lower(ATST.typedescription)!='closed' and ismandatory=true ;                                                          
