DROP FUNCTION IF EXISTS getchild114report(v_providerid integer, v_lipagesize bigint, v_lipagenumber bigint, date_sw character varying, date_from date, date_to date);

CREATE OR REPLACE FUNCTION cjams.getchild114report(v_providerid integer, v_lipagesize bigint, v_lipagenumber bigint, date_sw character varying DEFAULT NULL::character varying, date_from date DEFAULT NULL::date, date_to date DEFAULT NULL::date)
...
SELECT 
(CASE WHEN (RH.BALANCE_NO > 0) THEN 'YES' ELSE 'NO' END ) AS account_receivable_sw
,RH.BALANCE_NO AS account_receivable
...
(select value_tx from tb_picklist_values where picklist_type_id=2 and trim(picklist_value_cd)=trim(payhead.payment_type_cd) limit 1) as  payment_type_cd
...
inner join tb_receivable_header as rh  on rh.provider_id = prov.provider_id
...
prov.mail_code_tx,prov.tax_id_no,prov.prov_tax_type_cd,
prov.provider_nm,payment_status_nm
