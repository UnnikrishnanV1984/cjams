 CREATE OR REPLACE FUNCTION public.getgroupcase(userid character varying, isassigned boolean, groupsid uuid)         
  RETURNS TABLE(casedetail json)                                                                                     
  LANGUAGE plpgsql                                                                                                   
 AS $function$                                                                                                       
                                                                                                                     
 begin                                                                                                               
 return query                                                                                                        
 select  json_agg(ss) as users from   ( SELECT                                                                       
             ...
             ISR.intakeserviceid, cast(ISR.servicerequestnumber ||                                                   
              ...
              cast(case ISR.iscps                                                                                    
                   ...
                   else '' end  as character varying) cpsresponse,                                                   
             ...
             ISR.isrouted,                                                                                           
             R.Assignedon,                                                                                           
             cast ( coalesce(isr.offenselocation,'') as character varying) incidentlocation                          
     ...
     JOIN person as PN                                                                                               
       ...
       r.objectid = ISR.intakenumber                                                                                 
         INNER JOIN  IntakeServiceRequestGroupDetails ISGD ON ISGD.intakeserviceid  = ISR.intakeserviceid            
      AND ISGD.activeflag =1   and ISGD.groupid = groupsid                                                           
