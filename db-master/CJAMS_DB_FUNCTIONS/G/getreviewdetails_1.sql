...
CREATE OR REPLACE FUNCTION cjams.getreviewdetails(
...
RETURN    query    
	...
	SELECT   Count(1) over(),  
         ...
         a.appevent, 
         a.sdm,
         (select getcasepersonname as legalguardian from getcasepersonname ('servicerequest',a.intakeserviceid::character varying))
FROM     ( 
                    SELECT     
                               isr.intakeserviceid, 
                               ...
                               ( 
                                      ...
                                      LIMIT  1 ), isr.reporteddate :: timestamp, isra.clientname, a.updatedon :: timestamp AS asssignedon, cast( up.lastname
                                          || ',    ' 
...
ELSIF  (v_eventcode  =  'INVR')  THEN
	...
	SELECT   a.count, a.intakeserviceid, a.servicerequestnumber,a.intakenumber, a.intakeservreqtypekey, a.classkey, a.reporteddate, a.clientname, a.asssignedon, a.assingeduser, 
        ...
        FROM (  SELECT Count(1) over(), 
			       ...
			       GROUP BY isra.intakeserviceid, clientname ) isra ON  isra.intakeserviceid = isr.intakeserviceid 
