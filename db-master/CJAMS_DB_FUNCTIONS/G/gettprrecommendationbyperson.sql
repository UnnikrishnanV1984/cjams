...
-- Revision(s)
-- 07/22/2022 - Pratap - To retrieve tpr court details (CIDM-5026/B-130715)
------------------------------------------------------------------------
DROP FUNCTION IF EXISTS cjams.gettprrecommendationbyperson(v_servicecaseid uuid, v_permanencyplanid uuid, v_personid uuid);

CREATE OR REPLACE FUNCTION cjams.gettprrecommendationbyperson(v_servicecaseid uuid, v_permanencyplanid uuid, v_personid uuid)
...
AS $function$                                                                                                                                                                                                                                              
 ...
 RAISE NOTICE '%', v_permanencyplanid;
		...
		RETURN QUERY                                                              
		 ...
		 (SELECT ((            
				SELECT json_agg(d) FROM(
					SELECT isrp.petitionid,isrp.petitiontypekey,pt.description as petitiondesc,isrch.courtcasenumber, isrch.hearingtypekey, isrch.hearingtype ,ht.description as hearingtypedesc, 
						isrch.hearingdatetime,isrch.hearingstatustypekey, hst.description as hearingstatustypedesc,
						isrco1.hearingoutcometypekey,
						rfv.value_text  hearingoutcometypedesc, 
						( p.firstname || ' '||p.lastname):: character varying clientname, 
						isrp.petitiondate as petitiondate,
						isrco1.courtorderdate as courtorderdate,
						(SELECT 
								json_agg(
									jsonb_build_object(	
											'hearingclientid', hc2.hearingclientid
											, 'personid', hc2.personid
											, 'casenumber', hc2.courtcasenotx
											, 'clientname', (SELECT concat_ws(' ', p.prefx,p.firstname,p.middlename,p.lastname,p.suffix) FROM person p WHERE p.personid = hc2.personid)
											, 'otherclientflag', hc2.otherclientflag
											, 'annualnoticebenefitdt', hc2.annualnoticebenefitdt
											, 'intakeservicerequestactorid', (select isra.intakeservicerequestactorid from intakeservicerequestactor isra 
												where isra.personid = hc2.personid limit 1)		
									)
								) hearingclientdetails
							from hearingclients hc2 
							WHERE hc2.courthearingid = isrch.intakeservicerequestcourthearingid and hc2.activeflag = 1)
					from 
						intakeservicerequestcourthearing isrch
						join hearingclients hc on hc.courthearingid = isrch.intakeservicerequestcourthearingid 
						INNER JOIN intakeservicerequestpetition isrp ON isrch.intakeservicerequestpetitionid=isrp.intakeservicerequestpetitionid AND isrch.activeflag=1 
						inner join (select isrhoc.hearingoutcometypekey as hearingoutcometypekey, isrco.courtorderdate as courtorderdate, isrco.servicecaseid as servicecaseid
							from intakeservreqcourtorder isrco 
							join Intakeservreqcohearingoutcome isrhoc on isrco.intakeservreqcourtorderid = isrhoc.intakeservreqcourtorderid and isrhoc.activeflag = 1 and isrhoc.hearingoutcometypekey  in ('TPRGRA', 'TPRDEN')
						...
						LEFT  JOIN referencevalues rfv on rfv.ref_key = isrco1.hearingoutcometypekey and rfv.activeflag =1 and rfv.referencetypeid = 32
						LEFT JOIN person p ON p.personid=isrch.intakeservicerequestpetitionid AND p.activeflag=1     
						where isrch.servicecaseid  = v_servicecaseid
						and hc.personid = v_personid
    ...
    --Petition
	 ...
	 WHERE  isco.activeflag=1;
	RAISE  NOTICE  ' iscourtordere >>>>>>>>>>>> %',iscourtorder; 
