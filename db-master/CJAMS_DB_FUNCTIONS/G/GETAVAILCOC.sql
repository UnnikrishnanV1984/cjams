drop function if exists GETAVAILCOC (v_client_acc_id bigint);
CREATE OR REPLACE FUNCTION cjams.getavailcoc(v_client_acc_id bigint, date_sw character varying DEFAULT NULL::character varying, date_from date DEFAULT NULL::date, date_to date DEFAULT NULL::date)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
...
return (
 ...
 Select sum(A.coc) as AVAILABLE_BALANCE_NO_COC
from 
(  											
select (COALESCE((
