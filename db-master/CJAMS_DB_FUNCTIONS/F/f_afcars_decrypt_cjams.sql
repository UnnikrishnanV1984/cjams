CREATE OR REPLACE FUNCTION cjams.f_afcars_decrypt_cjams(VS_ENCRYPTED_ID character varying)
...
-- Date Created :02/25/2021 3:30 PM
...
BEGIN
	...
	WENNO1 := CAST((SUBSTR(LS_STEP2NO,1,1)) as INTEGER);
	WENNO2 := CAST((SUBSTR(LS_STEP2NO,2,1)) as INTEGER);
	WENNO3 := CAST((SUBSTR(LS_STEP2NO,3,1)) as INTEGER);
	WENNO4 := CAST((SUBSTR(LS_STEP2NO,4,1)) as INTEGER);
	WENNO5 := CAST((SUBSTR(LS_STEP2NO,5,1)) as INTEGER);
	WENNO6 := CAST((SUBSTR(LS_STEP2NO,6,1)) as INTEGER);
	WENNO7 := CAST((SUBSTR(LS_STEP2NO,7,1)) as INTEGER);
	WENNO8 := CAST((SUBSTR(LS_STEP2NO,8,1)) as INTEGER);
	WENNO9 := CAST((SUBSTR(LS_STEP2NO,9,1)) as INTEGER);
	WENNO10 := CAST((SUBSTR(LS_STEP2NO,10,1)) as INTEGER);
	WENNO11 := CAST((SUBSTR(LS_STEP2NO,11,1)) as INTEGER);
	WENNO12 := CAST((SUBSTR(LS_STEP2NO,12,1)) as INTEGER);
	...
	LS_STEP1NO =  Ltrim(rtrim(UWEN1)) || Ltrim(rtrim(UWEN2)) || Ltrim(rtrim(UWEN3)) || Ltrim(rtrim(UWEN4)) ||
	...
	-- STEP #1 : SEPERATE EACH NUMBER
	VENNO1 := CAST((SUBSTR(LS_STEP1NO,1,1)) AS INTEGER);
	VENNO2 := CAST((SUBSTR(LS_STEP1NO,2,1)) AS INTEGER);
	VENNO3 := CAST((SUBSTR(LS_STEP1NO,3,1)) AS INTEGER);
	VENNO4 := CAST((SUBSTR(LS_STEP1NO,4,1)) AS INTEGER);
	VENNO5 := CAST((SUBSTR(LS_STEP1NO,5,1)) AS INTEGER);
	VENNO6 := CAST((SUBSTR(LS_STEP1NO,6,1)) AS INTEGER);
	VENNO7 := CAST((SUBSTR(LS_STEP1NO,7,1)) AS INTEGER);
	VENNO8 := CAST((SUBSTR(LS_STEP1NO,8,1)) AS INTEGER);
	VENNO9 := CAST((SUBSTR(LS_STEP1NO,9,1)) AS INTEGER);
	VENNO10 := CAST((SUBSTR(LS_STEP1NO,10,1)) AS INTEGER);
	VENNO11 := CAST((SUBSTR(LS_STEP1NO,11,1)) AS INTEGER);
	VENNO12 := CAST((SUBSTR(LS_STEP1NO,12,1)) AS INTEGER);
	...
	-- 10th #
	IF Venn102 < 0 Then
		Uven10 := CAST((Venn102 + 10) AS INTEGER);
	Elsif Venn102 > 9 Then
		Uven10 := CAST((Venn102 - 10) AS INTEGER);
	Elseif Venn102 >= 0 AND Venn102 <= 9 Then
		Uven10 := CAST(Venn102 AS INTEGER);
	End if;
	
	-- 11th #
	IF Venn112 < 0 Then
		Uven11 := CAST((Venn112 + 10) AS INTEGER);
	Elsif Venn112 > 9 Then
		Uven11 := CAST((Venn112 - 10) AS INTEGER);
	Elseif Venn112 >= 0 AND Venn112 <= 9 Then
		Uven11 := CAST(Venn112 AS INTEGER);
	End if;
	
	-- 12th #
	IF Venn122 < 0 Then
		Uven12 := CAST((Venn122 + 10) AS INTEGER);
	Elsif Venn122 > 9 Then
		Uven12 := CAST((Venn122 - 10) AS INTEGER);
	Elseif Venn122 >= 0 AND Venn122 <= 9 Then
		Uven12 := CAST(Venn122 AS INTEGER);
	End if;
	...
	RAISE NOTICE 'LS_FINALDECRYPT Before %', LS_FINALDECRYPT;
	
	IF substring(LS_FINALDECRYPT,1,2) = '99' THEN
		VL_ENTITY_ID := substring(LS_FINALDECRYPT,3);
	ELSEIF substring(LS_FINALDECRYPT,1,1) = '9' THEN	
		VL_ENTITY_ID := substring(LS_FINALDECRYPT,2);
	ELSE
		VL_ENTITY_ID := LS_FINALDECRYPT;
	END IF;
	
	RAISE NOTICE 'LS_FINALDECRYPT After %', VL_ENTITY_ID;
