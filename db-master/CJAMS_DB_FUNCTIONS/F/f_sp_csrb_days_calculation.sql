...
CASE as_rate_flag
    WHEN 'R' THEN  -- Room and Board
      ...
      WHERE DELETE_SW = 'N'
          AND PARENT_KEY_ID = al_provider_id::character varying
    	...
    	/* SELECT f_sp_fostercare_calculation('L',
						...
						as_rfc_unit_type);-- */
						  ...
						  SELECT  a.ad_service_start_dt,
										a.ad_service_end_dt,
										a.al_unit_no,
										a.as_unit_type FROM F_SP_FOSTERCARE_CALCULATION(VS_L,
															VS_R,
															al_placement_structure_id,
															ad_placement_start_dt,
															ad_placement_end_dt,
	...
	OPEN CUR_PROVIDER_ADD FOR 
	   SELECT ADR_START_DT,ADR_END_DT,(CASE WHEN RTRIM(LTRIM(ADR_COUNTY_CD)) =  '1435' OR RTRIM(LTRIM(ADR_COUNTY_CD)) =  '1443' THEN 
	      	 	  'Y' 
	      		ELSE 'N'
	      		END) AS HIGHER_FC_RATE 
	      	    FROM tb_PROVIDER_ADDRESSES 
