-- FUNCTION: cjams.f_prim_worker_name_new(ai_case_id bigint)

-- DROP FUNCTION cjams.f_prim_worker_name_new(ai_case_id bigint);

CREATE OR REPLACE FUNCTION cjams.f_prim_worker_name_new(ai_case_id bigint)
...
BEGIN	
	...
	FROM servicecase 
	WHERE servicecasenumber = ai_case_id:: VARCHAR AND activeflag = 1;
...
VS_STAFF_NM = 
	(SELECT staff_nm FROM (
	SELECT row_number() over() AS num, staff_id, assignment_id, staff_nm FROM
	   (SELECT  userprofile.securityusersid AS staff_id, caseassignment.caseassignmentid AS assignment_id,
	   		(userprofile.lastname || ' ' || userprofile.firstname || COALESCE(',' || CASE WHEN userprofile.middlename = ' ' THEN NULL ELSE userprofile.middlename END,'')) AS staff_nm
	    FROM  caseassignment
		  	  , userprofile   
	     WHERE 	 
