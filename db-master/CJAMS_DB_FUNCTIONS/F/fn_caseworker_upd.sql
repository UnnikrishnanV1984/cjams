...
BEGIN
	...
	if	((new.toworkeridno <> new.fromworkeridno) and new.statustypekey='ASSGN')  then  
		...
		where caseassignment.objectid = servicecase.servicecaseid 
			and	caseassignment.toworkeridno <> caseassignment.fromworkeridno 
			and caseassignment.objecttypekey = 'servicecase' 
			and caseassignment.statustypekey = 'ASSGN' 
			and caseassignment.caseassignmentid = new.caseassignmentid 
		limit 1 ;
		...
		INSERT INTO csesoutboundtrigger	
			...
			from caseassignment, 
				adoptioncase 
			where caseassignment.objectid = adoptioncase.adoptioncaseid 
				and caseassignment.toworkeridno <> caseassignment.fromworkeridno 
				and caseassignment.objecttypekey = 'adoptioncase' 
				and caseassignment.objectid = new.objectid 
			limit 1 ;
	...
	if	((new.toldssid <> new.fromldssid) and new.statustypekey='ASSGN')  then  
		...
		INSERT INTO caresoutboundtrigger
			...
			from caseassignment,  
				servicecase 
			where caseassignment.objectid = servicecase.servicecaseid 
				and caseassignment.fromldssid <> caseassignment.toldssid 
				and caseassignment.objecttypekey = 'servicecase' 
				and caseassignment.statustypekey = 'ASSGN' 
				and caseassignment.caseassignmentid = new.caseassignmentid 
			limit 1 ;
