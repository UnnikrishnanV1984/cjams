CREATE OR REPLACE FUNCTION cjams.f_afcars_encrypt_chessie(vs_entity_id character varying)
...
BEGIN
	...
	IF vs_entity_id IS NULL OR vs_entity_id = '' THEN 
		LS_STEP1NO := '100' || '000000000';
	ELSE
		LS_STEP1NO := '100' || SUBSTRING('000000000',1,9 - LENGTH(LTRIM(RTRIM(vs_entity_id)))) || TRIM(vs_entity_id) ;
	END IF;
	...
	VENNO1 = CAST(SUBSTRING(LS_STEP1NO,4,1) AS INT);
	VENNO2 = CAST(SUBSTRING(LS_STEP1NO,5,1) AS INT);
	VENNO3 = CAST(SUBSTRING(LS_STEP1NO,6,1) AS INT);
	VENNO4 = CAST(SUBSTRING(LS_STEP1NO,7,1) AS INT);
	VENNO5 = CAST(SUBSTRING(LS_STEP1NO,8,1) AS INT);
	VENNO6 = CAST(SUBSTRING(LS_STEP1NO,9,1) AS INT);
	VENNO7 = CAST(SUBSTRING(LS_STEP1NO,10,1) AS INT);
	VENNO8 = CAST(SUBSTRING(LS_STEP1NO,11,1) AS INT);
	VENNO9 = CAST(SUBSTRING(LS_STEP1NO,12,1) AS INT);
	...
	LS_STEP2NO := '100' || Ltrim(rtrim(UVEN1)) || Ltrim(rtrim(UVEN2)) || Ltrim(rtrim(UVEN3)) || Ltrim(rtrim(UVEN4)) ||
	...
	-- STEP #1 : SEPERATE EACH NUMBER
	WENNO1 = CAST(SUBSTRING(LS_STEP2NO,4,1) AS INT);
	WENNO2 = CAST(SUBSTRING(LS_STEP2NO,5,1) AS INT);
	WENNO3 = CAST(SUBSTRING(LS_STEP2NO,6,1) AS INT);
	WENNO4 = CAST(SUBSTRING(LS_STEP2NO,7,1) AS INT);
	WENNO5 = CAST(SUBSTRING(LS_STEP2NO,8,1) AS INT);
	WENNO6 = CAST(SUBSTRING(LS_STEP2NO,9,1) AS INT);
	WENNO7 = CAST(SUBSTRING(LS_STEP2NO,10,1) AS INT);
	WENNO8 = CAST(SUBSTRING(LS_STEP2NO,11,1) AS INT);
	WENNO9 = CAST(SUBSTRING(LS_STEP2NO,12,1) AS INT);
	...
	LS_FINALENCRPT = '100' || Ltrim(rtrim(UWEN1)) || Ltrim(rtrim(UWEN2)) || Ltrim(rtrim(UWEN3)) || Ltrim(rtrim(UWEN4)) ||
