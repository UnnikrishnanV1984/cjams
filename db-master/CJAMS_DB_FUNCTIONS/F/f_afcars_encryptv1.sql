	CREATE OR REPLACE FUNCTION F_AFCARS_ENCRYPTV1 ( VS_CISCLIENT_ID Varchar (12))
	------------------------------------------------------------------------
	-- SQL Stored Procedure
	-- Encrypt CIS Client Id  in AFCARS
	------------------------------------------------------------------------
...
--     DECLARE VL_OUTPUT_SQLCODE INTEGER DEFAULT 0;--
	...
	DECLARE VL_ENCRYPT_CISCLIENT_ID VARCHAR(12);--
	Declare LS_STEP1NO  VARCHAR (12) default null;--
	Declare LS_STEP2NO  VARCHAR (12) default null;--
	Declare LS_FINALENCRPT VARCHAR (12) DEFAULT null;--
...
BEGIN --atomic
		...
		LS_STEP1NO := '100' || SUBSTR('000000000',1,9 - LENGTH(LTRIM(RTRIM(VS_cisclient_id)))) || VS_cisclient_id ;--
	   ...
	   IF Wenn32 < 0 Then
		...
		VL_ENCRYPT_CISCLIENT_ID =   LS_FINALENCRPT;--
	RETURN VL_ENCRYPT_CISCLIENT_ID;--
	END $$;
