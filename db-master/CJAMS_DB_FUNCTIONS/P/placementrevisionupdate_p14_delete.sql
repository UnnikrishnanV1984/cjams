Drop FUNCTION cjams.placementrevisionupdate(	character varying, 
  ...
  -- if v_count > 0  then 
   ...
   raise notice 'v_routingstatustypeid>>>>>>>>>>>> %',v_routingstatustypeid;
	...
	INSERT INTO cjams.placementrevision
		...
		voidreasontypekey,voidremarks,exittypekey,remarks,isvoided,voiddate,requestedby,requesteddate,
		ischangepreadoptive)
	VALUES(v_placementid:: uuid, now()::date, coalesce((v_startdate :: date),(p_startdate :: date),null), coalesce(v_starttime,p_starttime,null), (v_enddate :: date)
		...
		v_exittypekey  ,v_remarks,v_isvoid,v_voiddate ,securityuserid,now(),
		v_ischangepreadoptive);
