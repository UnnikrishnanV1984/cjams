DROP FUNCTION IF EXISTS cjams.expungscreenoutintakes();

CREATE OR REPLACE FUNCTION cjams.expungscreenoutintakes(
	)
...
BEGIN
	...
	SELECT generate_series(
			COALESCE((SELECT MAX(rundate) + INTERVAL '1 day' lastrundate FROM expungementstatus WHERE jobid = 1), NOW()::DATE), 
			NOW()::DATE, 
			'1 day'), 'pending'; 
	...
	FOR v_brundate, v_bstatus IN SELECT * FROM tmp_batchtorun
		LOOP
			...
			PERFORM expungscreenoutintakes(v_brundate);
			UPDATE tmp_batchtorun SET status = 'SUCCESS' WHERE batchrundate = v_brundate;
			INSERT INTO expungementstatus (jobid, rundate, status, starttime, endtime, expungscreenouts, expungperson) 
				VALUES (1, v_brundate, 'SUCCESS', v_starttime, NOW()
						, (SELECT JSON_AGG(T) FROM (SELECT * FROM  tmp_expungscreenout) T) 
