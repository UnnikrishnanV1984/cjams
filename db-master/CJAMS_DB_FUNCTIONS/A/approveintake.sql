DROP FUNCTION IF EXISTS approveintake(json, json);
CREATE OR REPLACE FUNCTION approveintake(
	savedtlsobj json,
	reviewobj json)
    RETURNS TABLE(responseservicereqnum character varying, responseintakeserviceid uuid, message character varying, intakeservicerequestactorid character varying, isfamilycase integer) 
      LANGUAGE plpgsql                                                                                                                                                                                                                          
 AS $function$  
...
LOOP 
	...
	IF ((i->>    'DAStatus') = 'Approved' and lower(i->>    'DADisposition') = 'screenout') THEN 
	v_status_new := true;
	END IF;	 
	END IF;
END    LOOP;
...
LOOP
   ...
   /*IF (v_peaceorder  = 'PORA' or v_peaceorder = 'FPTSAO') THEN 
    SELECT focuspersonid,raname FROM intakedastaging INTO v_Pid,v_focuspersonname WHERE intakenumber = v_intakeNumber AND activeflag=1 LIMIT 1;
	 
	IF (v_suppeaceorderdesc  = 'Request Not Authorized') THEN
	v_alertnotes = 'Peace Order – Request Not Authorized is set for '||v_focuspersonname ||' in Intake '||  v_intakeNumber;
	SELECT savepersonalert INTO v_savepersonalert FROM savepersonalert('PONA'::character varying,'Active'::character varying,now()::timestamp without time zone,v_alertnotes::text,v_Pid::uuid,v_securityuserid::character varying,null::uuid,(now()+ interval '90 day')::date);
	END IF;
	IF (v_suppeaceorderdesc  = 'Request Authorized') THEN
	v_alertnotes = 'Peace Order – Request Authorized is set for '||v_focuspersonname ||' in Intake '||  v_intakeNumber;
	SELECT savepersonalert INTO v_savepersonalert FROM savepersonalert('POAU'::character varying,'Active'::character varying,now()::timestamp without time zone,v_alertnotes::text,v_Pid::uuid,v_securityuserid::character varying);
	END IF;
	END IF; */ -- Gavaskar 28032019
	...
	IF (v_Agencycode = 'CW') THEN 
        IF ((j->>    'DAStatus') = 'Approved' and lower(j->>    'DADisposition') = 'screenout') THEN
...
LOOP 
    ...
    IF    length(i    ->>    'Pid')    >    1    and        left((i->>'Pid'),6)        <>    'tempid'    THEN
        UPDATE person 
        SET    dangerlevel = CASE Lower(i ->> 'Dangerousworker')WHEN 'yes' THEN 1 WHEN 'no' THEN 0 ELSE NULL END, 
           ...
           dob = ( i ->> 'Dob' ) :: DATE, 
           dateofdeath = ( i ->> 'dateofdeath' ) :: DATE, 
           isapproxdod = ( i ->> 'isapproxdod' ) :: INT, 
           isapproxdob = ( i ->> 'isapproxdob' ) :: INT, 
           religiontypekey = i ->> 'religionkey', 
           tribalassociation = i ->> 'tribalassociation', 
           physicalattributes = i ->> 'physicalattributes', 
           strengths = i ->> 'strengths', 
           needs = i ->> 'needs', 
           livingsituationkey = i ->> 'livingsituationkey', 
           licensedfacilitykey = i ->> 'licensedfacilitykey', 
           otherlicensedfacility = i ->> 'otherlicensedfacility', 
           livingsituationdesc = i ->> 'livingsituationdesc', 
           primarylanguageid = i ->> 'primarylanguage', 
           otherprimarylanguagetypekey = i ->> 'otherprimarylanguage', 
           otherreligion = i ->> 'otherreligion', 
           citizenalenageflag = ( i ->> 'citizenalenageflag' ) :: INT, 
           othergendertypekey = ( i ->> 'othergendertypekey' ) :: INT, 
           isqualifiedalien = ( i ->> 'isqualifiedalien' ) :: INT, 
           alienregistrationtext = i ->> 'alienregistrationtext', 
           verificationremarks = i ->> 'verificationremarks', 
           alienstatustypekey = i ->> 'alienstatustypekey', 
           everbeenadoptedflag = ( i ->> 'everbeenadoptedflag' ) :: INT 
	...
	ELSE    
            ...
            SELECT 1, 
               ...
               tribalassociation, 
               physicalattributes, 
               CASE Lower(dangerousself) WHEN 'yes' THEN 1 ELSE 0 END, 
               dangerousselfreason, 
               CASE Lower(dangerousworker) WHEN 'yes' THEN 1 ELSE 0 END, 
               dangerousworkerreason, 
               v_securityuserid, 
               v_date, 
               v_securityuserid, 
	...
	/* Emergency Contact */
	v_emergencycontact  :=  i->>  'emergency';	
	FOR emergencycontact IN SELECT  * 	FROM   Json_array_elements(v_emergencycontact) 
	LOOP
        INSERT INTO emergencycontactperson 
        ...
        VALUES      ( Gen_random_uuid(), 
         l_generatedpersonid, 
         ( emergencycontact ->> 'contactpersonid' ) :: uuid, 
         1, 
         v_securityuserid, 
         v_date, 
         v_securityuserid, 
	...
	UPDATE personaddress pa SET    activeflag = 0, updatedby = v_securityuserid, updatedon = v_date 
	WHERE  personaddressid NOT IN (SELECT addressid :: uuid FROM   sp_createintake_temp_address 
	   WHERE  isnew = 0) 
	AND pa.personid = l_generatedpersonid; 	
	
	
	UPDATE personphonenumber pa SET    activeflag=0, updatedby = v_securityuserid, updatedon = v_date 
	WHERE  personphonenumberid NOT IN ( SELECT contacttypeid::uuid FROM   sp_createintake_temp_personcontact	WHERE isnew=0) 
	AND pa.personid = l_generatedpersonid ;	  
                                                                                          
	UPDATE personemail pa SET    activeflag = 0, updatedby = v_securityuserid, updatedon = v_date 
	WHERE  personemailid IN (SELECT mailtypeid :: uuid FROM   sp_createintake_temp_personmail WHERE  isnew = 0) 
	AND pa.personid = l_generatedpersonid;                          
	
	/*Inactive and Alias added to the person*/
	...
	SELECT l_generatedpersonid, 
	   1, 
	   COALESCE(addresstype, 'C'), 
	   COALESCE(address1, '    '), 
	   address2, 
	   zipcode, 
	   COALESCE(city, ''), 
	   ...
	   v_securityuserid, 
	   v_date, 
	   COALESCE(startdate, v_date), 
	   enddate, 
	   CASE Lower(knowndangeraddress) 
	 WHEN 'yes' THEN true 
	 WHEN 'no' THEN false 
	 ELSE NULL 
	   END, 
      ...
      COALESCE(startdate, v_date), 
	   enddate	   
	FROM   sp_createintake_temp_address 
	WHERE  isnew = 1;      
	/*Phone number*/
	...
	SELECT l_generatedpersonid, 
	   ...
	   v_date, 
	   v_date 
	FROM   sp_createintake_temp_personcontact 
	WHERE  isnew = 1; 
	/*Email */
	...
	SELECT l_generatedpersonid, 
	   ...
	   v_date, 
	   v_date 
	FROM   sp_createintake_temp_personmail 
	WHERE  isnew = 1; 	
	
	IF  LENGTH(LOWER(i->>'SSN'))    >    0    THEN    
        UPDATE personidentifier SET    activeflag = 0 WHERE  personid = l_generatedpersonid AND personidentifiertypekey = 'SSN'; 
        INSERT INTO personidentifier(personid, personidentifiertypekey, personidentifiervalue, insertedby, insertedon, activeflag, effectivedate) VALUES (l_generatedpersonid,'SSN', i->>'SSN',v_securityuserid,v_date,1,v_date);                                                                     
	END    IF;
	IF Length(Lower(i ->> 'dl')) > 0 then 
        UPDATE personidentifier SET    activeflag =0 WHERE  personid= l_generatedpersonid AND    personidentifiertypekey ='DL';
        ...
        VALUES 
        ( 
        l_generatedpersonid,'DL', i ->> 'dl', 
        v_securityuserid, v_date, 1, v_date 
        );
	 END IF;    
	
	IF   Length(LOWER(i    ->>    'height'))    >    0    THEN    
	UPDATE personphysicalattribute SET    activeflag = 0 WHERE  personid = l_generatedpersonid AND physicalattributetypekey = 'Ht';  
	...
	IF  Length(LOWER(i    ->>    'weight'))    >    0    THEN    
	UPDATE personphysicalattribute SET    activeflag = 0 WHERE  personid = l_generatedpersonid AND physicalattributetypekey = 'Wt'; 
        INSERT INTO personphysicalattribute 
        (personid, 
         physicalattributetypekey, 
         attributevalue, 
	...
	IF  Length(LOWER(i    ->>    'tattoo'))    >    0    THEN    
        UPDATE personphysicalattribute SET    activeflag = 0 WHERE  personid = l_generatedpersonid AND physicalattributetypekey = 'Tattoo'; 
        INSERT INTO personphysicalattribute 
        (personid, 
         physicalattributetypekey, 
         attributevalue, 
	...
	IF  Length(LOWER(i    ->>    'PhyMark'))    >    0    THEN    
        UPDATE personphysicalattribute SET    activeflag = 0 WHERE  personid = l_generatedpersonid AND physicalattributetypekey = 'PhyMark'; 
        INSERT INTO personphysicalattribute 
        (personid, 
         physicalattributetypekey, 
         attributevalue, 
...
IF (SELECT Count(1) FROM   sp_createintake_temp_dageneral DAG  WHERE  DAG.ananymousreporter LIKE '%true%') = 1 THEN
	...
	SELECT v_reporterpersonid, 
	   ...
	   v_securityuserid, 
	   v_date, 
	   v_date;v_ReporterActorID = gen_random_uuid() ;
...
LOOP  
	...
	UPDATE assessment SET    objectid = v_intakeserviceid,
     updatedby = v_securityuserid,                                                                                                                                                                                                                                                          
     updatedon = v_date 
     WHERE  intakenumber = v_intakenumber AND    COALESCE(objectid:: character VARYING,'') ='00000000-0000-0000-0000-000000000000';                                                    
	END IF;
...
/*
	...
	INSERT INTO intakeservicerequestservice (intakeserviceid, teamtypekey, intakeservreqservicekey , insertedby, updatedby )
	SELECT    v_IntakeServiceId,    *,v_securityuserid,v_securityuserid 
...
--For    Inserting    Agency    Details 
    ...
    IF  (v_Agencycode='AS')    THEN 
        SELECT    createadultscreentool  INTO  v_Adultscreentoolreturn   FROM    createadultscreentool(saveDtlsObj::json,v_IntakeServiceId::uuid,v_securityuserid);
    END IF ;	
...
IF (v_Agencycode='DJS') THEN 
	SELECT r.roletypekey INTO v_roletypekey FROM muser mu 
	JOIN rolemapping rm on rm.principalid = cast(mu.id as character varying) AND rm.activeflag = 1
	JOIN role r on r.id = rm.roleid AND  r.activeflag = 1
	WHERE mu.securityusersid = v_securityuserid AND  mu.activeflag = 1 ;
	
	SELECT DA.IntakeServiceId INTO v_tempintakeserviceid FROM sp_CreateIntake_Temp_DAType DA WHERE    DA.ServiceRequestNumber = v_DAServiceRequestNumber;
	
	SELECT createtargetstatusdisposition INTO v_created_isrdispositioncodeid FROM createtargetstatusdisposition(v_dispositionid, v_subdispositionid::uuid, v_tempintakeserviceid, v_roletypekey, v_securityuserid, null);
    -- FocusPersonStatus Insert
	SELECT getfocuspersoncasestatusconfig INTO v_savefocuspersoncasestatus 
	FROM getfocuspersoncasestatusconfig(l_focuspersonid, v_tempintakeserviceid , v_intakeNumber,null, v_dispositionid, v_subdispositionid::uuid, v_securityuserid); 
	-- Focus Person Case More Details Insert
    IF (j_focuspersoncasedetails is not null) THEN
        SELECT savefocuspersoncasedetails INTO v_savefocuspersoncasedetails FROM savefocuspersoncasedetails(v_tempintakeserviceid::uuid,v_intakeNumber,j_focuspersoncasedetails,v_securityuserid);
    END IF;             
END If;
	...
	case        DAP.iscollateral    when    true    THEN    1    else    0    end    ,
                                                ...
                                                DAP.fetalalcoholspctrmdisordflag, 
                                                DAP.drugexposednewbornflag, 
                                                DAP.probationsearchconductedflag,  
                                                DAP.sexoffenderregisteredflag         
...
LOOP
	...
	v_personrole    =    v_personrolerecord.personrole;
                    ...
                    SELECT    ActorID,Personid,fetalalcoholspctrmdisordflag,drugexposednewbornflag,probationsearchconductedflag,sexoffenderregisteredflag    INTO    v_ActorID        ,    v_actpersonid,v_fetalalcoholspctrmdisordflag,v_drugexposednewbornflag,v_probationsearchconductedflag,v_sexoffenderregisteredflag	 
	...
	v_actpersonid ,
	v_fetalalcoholspctrmdisordflag,
	v_drugexposednewbornflag,
	v_probationsearchconductedflag,
	v_sexoffenderregisteredflag                                                               
...
----    Added    evaluation    by    Gavaskar        28-08-2018
	...
	IF(v_Agencycode='DJS' or  v_Agencycode = 'AS')	THEN
	
	IF(v_Agencycode='DJS')THEN
	
	update intakedastaging set focuspersonid=( select PN.personid from person as PN
    join actor as A on A.personid= PN.personid 
    join intakeservicerequestactor as ISRA on ISRA.actorid = a.actorid and lower(ISRA.intakeservicerequestpersontypekey) = 'youth'
    join intakeservicerequest as ISR on ISR.intakeserviceid= ISRA.intakeserviceid
    where ISR.intakenumber = v_intakeNumber limit 1 )  WHERE intakenumber = v_intakeNumber and activeflag=1 ;
	...
	-- FocusPersonStatus Insert
	--SELECT getfocuspersoncasestatusconfig INTO v_savefocuspersoncasestatus 
	--FROM getfocuspersoncasestatusconfig((SELECT focuspersonid FROM intakedastaging WHERE intakenumber = v_intakeNumber AND activeflag=1 LIMIT 1), v_tempintakeserviceid , v_intakeNumber,null, v_dispositionid, v_subdispositionid::uuid, v_securityuserid);
	...
	IF(l_draftCount=0) THEN
                             ...
                             INSERT INTO intakedastaging(intakenumber, dateRecieved, narrative, raname, entityname,  intakeuser, cruworkername, data, insertedon, 
                             insertedby, timerecieved, updatedon, updatedby, status,jsondata,versionnumber,dispositiondescription,statusdescription,ispreintake,isclw,focuspersonid)
                             VALUES (v_InatkeNumber,  v_date  , null, l_RAName, null, v_securityuserid,v_securityuserid, null, 
                             v_date, v_securityuserid, v_date,  v_date, null, 'pending',savedtlsobj,1,v_dadispositiontext,v_dastatustext,
                             l_ispreintake,v_isclw,l_focuspersonid);
	...
	--if(v_intservreqtypkey = 'Waiver FROM Adult Court') -- comment by Gavaskar 20-02-2019
	if(v_Agencycode='DJS') -- added by Gavaskar 20-02-2019
	THEN
	   SELECT saveevaluation INTO v_evaluationstatus FROM saveevaluation(v_InatkeNumber, j_evaluation_fields, v_securityuserid);    
                       END IF;
	       END IF;
	...
	SELECT    gen_random_uuid(),    RA.personid,    p.personid,    p.RelationshiptoRA,    v_securityuserid,    v_date,    v_securityuserid,    v_date

	FROM    sp_createintake_temp_daperson    p    CROSS    JOIN    sp_createintake_temp_daperson    RA

	WHERE    LOWER(p.actorroletype)    not    in    ('ra',    'rc',    'youth')    AND    LOWER(RA.actorroletype)    in    ('ra',    'rc',    'youth')    

	AND    CAST(p.personid    AS    character    VARYING)    ||    ','    ||    CAST(RA.personid    AS    character    VARYING)    ||    ','    ||    RA.RelationshiptoRA    NOT    IN    (

	SELECT    CAST(pr.personid    AS    character    VARYING)    ||    ','    ||    CAST(pr.personrelativeid    AS    character    VARYING)    ||    ','    ||    pr.actorrelationshipkey    FROM    personrelation    pr    WHERE    activeflag    =    1
...
LOOP
	...
	update intakeservicerequestactor isst set ismaltreator= true WHERE isst.intakeservicerequestactorid in (SELECT isra.intakeservicerequestactorid  FROM  intakeservicerequestactor  isra  join  actor  a  on  a.actorid  =  isra.actorid
	...
	LOOP
	 ...
	 INSERT  INTO  investigationmaltreatment(investigationid,isjurisdiction, isreported,activeflag,insertedby,effectivedate) VALUES  (v_investigationid,0,true,1,  v_securityuserid,  v_date) RETURNING  "maltreatmentid"  INTO  l_newmaltreatementid;
	RAISE  NOTICE  '  maltreatmentid  %',l_newmaltreatementid;
	 
	INSERT  INTO  investigationallegation(

                        investigationid,  allegationid, maltreatmentid, name,  
	...
	INSERT  INTO  investigationallegationindicator(
                        ...
                        investigationallegationid,  indicatorid,  

                         activeflag,insertedby)

	VALUES  (l_newinvestigationallegationid,  (indicators->>'indicatorid')::uuid,    
...
/*Update    Intake    status*/                    
 ...
 IF    v_isDisposition    =    true    THEN 
	...
	v_securityuserid,    'Complete' ,jsondata,versionnumber,dispositiondescription,statusdescription,ispreintake,true,v_sstastatustypekey    --,1    
	...
	UPDATE folderassignment    SET    status='Closed',closedate=now(),activeflag=0, updatedon= now(), updatedby=v_securityuserid    WHERE    objectid=v_intakeNumber    AND    foldertypekey='Intake'    AND    status='Active';

	INSERT    INTO    folderassignment(folderassignmentid,    objectid,    foldertypekey,    status,    opendate,        activeflag,    effectivedate, updatedon, insertedon, insertedby, updatedby)
