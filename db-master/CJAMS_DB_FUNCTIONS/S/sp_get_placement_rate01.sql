CREATE OR REPLACE FUNCTION cjams.sp_get_placement_rate(al_placement_structure_id bigint, al_child_age integer, ad_start_dt timestamp without time zone, ad_end_dt timestamp without time zone, as_type_cd character, OUT adc_gross_amount numeric, OUT adc_per_diem_rate numeric, OUT al_sqlcode integer, OUT as_error character varying)
 RETURNS record
 LANGUAGE plpgsql
AS $function$
...
--    vl_sqlcode := -1 ;--
--    as_error := COALESCE(as_error ,'') || CHAR( CURRENT_TIMESTAMP) ||'::' || vs_Procedure_nm || '.' ;--
--    as_error := COALESCE(as_error ,'') || '::RO ' || 'Placement Structure_id' || ' :: ' ||
--COALESCE(char(al_placement_structure_id),'');--
...
--FROM sysibm.sysdummy1;--
         ...
         WHEN 'R' THEN  -- room and board
            ...
            SELECT MONTHLY_RATE_NO,
              ...
              INTO adc_gross_amount,
                   adc_per_diem_rate
              FROM cjams.TB_FOSTER_CARE_RATE
 ...
 WHEN 'P' THEN  -- private provider
        ...
        SELECT MONTHLY_RATE_NO,
          ...
          INTO adc_gross_amount,
               adc_per_diem_rate
           FROM cjams.TB_PROV_PROGRAM_RATES
