CREATE OR REPLACE FUNCTION cjams.sp_under_over_adoption(ad_run_dt date, OUT return_code integer, OUT al_sqlcode integer, OUT as_error character varying)
 RETURNS record
 LANGUAGE plpgsql
AS $function$
------------------------------------------------------------------------
...
--				  To create under/over Adoption Subsidy payments due to retroactive changes to following elements:
--					Payment Amount
--					Rate Start Date
--					Rate End Date
...
-- 06/24/2020 Vineet Tirodkar - To fix the Query column name under Adoption Subsidy Suspension logic
...
-- 06/28/2021 Vineet Tirodkar - Changes the order by clause of rate cursor to update most recent unit rate in payment table (CDM-14135)
-- 09/30/2022 - Vineet Tirodkar - To char fix for Aurora DB migration 
-- 05/12/2023 - Vineet Tirodkar - To fix function month(date) error (CDM-31194)
...
DECLARE vs_Procedure_nm VARCHAR(100) DEFAULT 'SP_UNDER_OVER_ADOPTION';
DECLARE vs_identity_column VARCHAR(100);
DECLARE vs_identity_val VARCHAR(100);

DECLARE vl_rate_structure_id	INTEGER DEFAULT 501; --	Adoptive Home
...
DECLARE vl_adop_payment_cnt			INTEGER DEFAULT 0;
...
CUR_UNDER_OVER_ADOPTION record;
 CUR_SUSPENSION_CHANGES record;
 CUR_UNDER_OVER_ADOPTION_refcur REFCURSOR;
...
begin
	...
	return;
END;
raise notice '139>>>> under over adoption';
--END;
-- Log error

-- Check for Monthly Adoption Subsidy payment batch run (DRAFT Payment) - START 
-- DRAFT Payment must run prior to Adoption Subsidy under/over batch
...
IF date_part('DAY', ad_run_dt ) = 1 OR date_part('DAY', current_date) = 1 THEN
	...
	IF vl_draft_payment_run = 0 OR vl_adop_payment_cnt = 0 THEN
		as_error := 'Monthly Adoption Subsidy payment batch must run prior to the Adoption Subsidy under/over batch.' ;
...
raise notice '174>>>>>>>>>>SKIP_BATCH_RUN';
SELECT (date_trunc('month', now())::date - 1)
      
INTO vd_previous_month_end_dt;
        

-- Adoption Subsidy Under/Over main cursor - START
--FOR CUR_UNDER_OVER_ADOPTION AS
--cur1 CURSOR WITH HOLD FOR
OPEN CUR_UNDER_OVER_ADOPTION_refcur FOR
	SELECT TAS.ADOPTION_ID, 
			TAS.PROVIDER_ID,
			TA.CLIENT_ID,
			TA.SUBSIDY_START_DT,
			TA.SUBSIDY_END_DT,
			F_PRIM_COUNTY(TA.CASE_ID,'NULL',TA.CLIENT_ID,'CLIENT') AS COUNTY_CD,
			TAS.AGREEMENT_START_DT AS CHANGE_START_DT, 
			TAS.AGREEMENT_END_DT AS CHANGE_END_DT,
			'RATE_CHANGES' AS TRAN_TYPE,
			TAS.adoptionagreementrateid AS PARENT_KEY_ID
		FROM tb_ADOPTION_SUBSIDY_AGREEMENT TAS,
			 tb_ADOPTION TA
	WHERE TAS.ADOPTION_ID = TA.ADOPTION_ID
		AND TAS.DELETE_SW = 'N'
		AND TA.DELETE_SW = 'N'
		AND TAS.adoptionagreementrateid IN 
			-- VINEET 12102019 (added new and commented existing)
			-- New
			(SELECT agr.adoptionagreementrateid											
				FROM cjams.adoptioncaserevision AR, cjams.adoptioncaseagreementrate AGR
			 WHERE ar.adoptionagreementrateid = AGR.adoptionagreementrateid
				and AR.activeflag = 1
				and COALESCE(AR.approvalstatustypekey, '') = '3047'
				and AR.approvaldate::date = ad_run_dt::date
				and btrim(lower(AGR.status::text)) = 'approved'::text
				and AGR.activeflag = 1 
			union -- TANMAY 07092020 (for new cases without any revision)
			SELECT agr.adoptionagreementrateid
				FROM cjams.adoptioncaseagreementrate AGR
			WHERE btrim(lower(AGR.status::text)) = 'approved'::text
				and AGR.activeflag = 1
				and (AGR.approvaldate::date = ad_run_dt::date or AGR.updatedon::date = ad_run_dt::date)
			)
			-- Existing
			/*
			(SELECT agr.adoptionagreementrateid											
				FROM cjams.adoptioncaserevision AR, cjams.adoptioncaseagreementrate AGR
				WHERE ar.adoptionagreementid = AGR.adoptionagreementid
				and AR.activeflag = 1
				and COALESCE(AR.approvalstatustypekey, '') = '3047'
				and AR.approvaldate::date = ad_run_dt::date
				and btrim(lower(AGR.status::text)) = 'approved'::text
	...
	SELECT TA.ADOPTION_ID, 
			-- TA.PROVIDER_ID,
			(select TAS.PROVIDER_ID from tb_ADOPTION_SUBSIDY_AGREEMENT TAS where TAS.ADOPTION_ID = TA.ADOPTION_ID
				AND TAS.DELETE_SW = 'N'
				AND COALESCE(TAS.APPROVAL_STATUS_CD, '') = '3047'
			ORDER BY TAS.agreement_start_dt DESC LIMIT 1) AS PROVIDER_ID,
			TA.CLIENT_ID,
			TA.SUBSIDY_START_DT,
			TA.SUBSIDY_END_DT,
			F_PRIM_COUNTY(TA.CASE_ID,'NULL',TA.CLIENT_ID,'CLIENT') AS COUNTY_CD,
			ASP.suspensionbegindate AS CHANGE_START_DT, 
			ASP.suspensionenddate AS CHANGE_END_DT,
			'SUSPENSION_CHANGES' AS TRAN_TYPE,
			ASP.adoptionsuspensionid AS PARENT_KEY_ID
		FROM adoptioncasesuspension ASP,		
			 tb_ADOPTION TA
	WHERE ASP.adoptioncaseid = TA.adoption_case_id
		-- AND ASP.DELETE_SW = 'N'
		AND ASP.activeflag = 1
		AND TA.DELETE_SW = 'N'
		and ASP.suspensionbegindate is NOT NULL
		AND ASP.adoptionsuspensionid IN ( SELECT ASR.adoptionsuspensionid
										-- FROM tb_ADOPTION_SUSPENSION_REVISION ASR
										FROM adoptioncasesuspensionrevision ASR
								    WHERE ASR.activeflag = 1
										   AND COALESCE(ASR.approvalstatustypekey, '') = '3047'
										   AND ASR.approvaldate::date = ad_run_dt::date
										   and ASR.suspensionbegindate is NOT NULL
									   -- for Unit Testing
									   -- AND ASR.ADOPTION_ID = ??
									   -- for Unit Testing
								  )
...
--DO
loop
raise notice '240>>>>>>>>insdie loop';
	fetch CUR_UNDER_OVER_ADOPTION_refcur into CUR_UNDER_OVER_ADOPTION;
										 exit when not found;
	...
	-- INITIAL VALUES
	 ...
	 vl_adoption_id := CUR_UNDER_OVER_ADOPTION.ADOPTION_ID;
	 vl_provider_id := CUR_UNDER_OVER_ADOPTION.PROVIDER_ID;
	 vs_county_cd := CUR_UNDER_OVER_ADOPTION.COUNTY_CD;
	 vl_client_id := CUR_UNDER_OVER_ADOPTION.CLIENT_ID;
	 vd_change_start_dt := CUR_UNDER_OVER_ADOPTION.CHANGE_START_DT;
	 vd_change_end_dt := CUR_UNDER_OVER_ADOPTION.CHANGE_END_DT;
	 vd_subsidy_start_dt := CUR_UNDER_OVER_ADOPTION.SUBSIDY_START_DT;
	 vd_org_subsidy_end_dt := CUR_UNDER_OVER_ADOPTION.SUBSIDY_END_DT;
	 vs_TRAN_TYPE := CUR_UNDER_OVER_ADOPTION.TRAN_TYPE;
	 vl_parent_key_id := CUR_UNDER_OVER_ADOPTION.PARENT_KEY_ID;
	raise notice '272 >>>>>>vl_adoption_id %',vl_adoption_id;
	-- Set Change Type for Fiscal Audit Trail
	IF vs_TRAN_TYPE = 'RATE_CHANGES' THEN
		 vs_change_type := '1013'; --	Adoption Subsidy Rate Changes
	ELSE -- SUSPENSION_CHANGES
		 vs_change_type := '1014'; --	Adoption Subsidy Suspension Changes
	END IF;
	...
	IF vs_TRAN_TYPE = 'SUSPENSION_CHANGES' THEN
		...
		-- SELECT MIN(ADOPTION_SUSPENSION_REVISION_ID)
		...
		SELECT adoptionsuspensionrevisionid
			INTO vl_first_susp_change_id
		FROM adoptioncasesuspensionrevision
		WHERE adoptionsuspensionid = vl_parent_key_id 
			AND activeflag = 1
			and suspensionbegindate is NOT NULL
			AND COALESCE(approvalstatustypekey, '') = '3047'
			order by suspensionbegindate limit 1;
		
		 al_sqlcode := SQLCODE;
		IF al_sqlcode < 0 THEN
			 as_error := 'Error in getting MIN(ADOPTION_SUSPENSION_REVISION_ID) from tb_ADOPTION_SUSPENSION_REVISION'  ;
		...
		-- 	ORDER BY ADOPTION_SUSPENSION_REVISION_ID ;
		OPEN CUR_SUSPENSION_CHANGES_refcur FOR
			SELECT adoptionsuspensionrevisionid, suspensionbegindate
				FROM adoptioncasesuspensionrevision
			WHERE adoptionsuspensionid = vl_parent_key_id 
				AND activeflag = 1
				and suspensionbegindate is NOT NULL
	...
	fetch CUR_SUSPENSION_CHANGES_refcur into CUR_SUSPENSION_CHANGES;
			...
			vl_suspension_revision_id := CUR_SUSPENSION_CHANGES.adoptionsuspensionrevisionid;
			vd_start_date := CUR_SUSPENSION_CHANGES.suspensionbegindate;
			
			-- First Suspension Change - check against ORIGINAL_SW = 'Y'
			IF vl_first_susp_change_id = vl_suspension_revision_id THEN 
				-- SELECT SUSPENSION_BEGIN_DT
				...
				WHERE adoptionsuspensionid = vl_parent_key_id 
					...
					AND activeflag = 1
					and suspensionbegindate is NOT NULL
					AND suspensionbegindate::date < vd_start_date::date
					ORDER BY suspensionbegindate limit 1;
					
				 al_sqlcode := SQLCODE;
				IF al_sqlcode < 0 THEN
					 as_error := 'Error in getting SUSPENSION_BEGIN_DT from tb_ADOPTION_SUSPENSION_REVISION (ORIGINAL_SW = Y)'  ;
					 vs_identity_column := 'SUSPENSION_ID';
					 vs_identity_val := (vl_parent_key_id)::character varying;
					--SIGNAL p_sp_error  ;
				END IF ;				
			ELSE
				...
				-- FROM tb_ADOPTION_SUSPENSION_REVISION 	
				-- WHERE SUSPENSION_ID = vl_parent_key_id 
				-- 	AND ADOPTION_SUSPENSION_REVISION_ID < vl_suspension_revision_id
				-- 	AND APPROVAL_STATUS_CD = '3047'
				-- 	AND DELETE_SW = 'N' 	
				-- 	AND COALESCE(ORIGINAL_SW, 'N') <> 'Y'
				...
				ORDER BY suspensionbegindate limit 1;
				 ...
				 al_sqlcode := SQLCODE;
				IF al_sqlcode < 0 THEN
					 as_error := 'Error in getting SUSPENSION_BEGIN_DT from tb_ADOPTION_SUSPENSION_REVISION'  ;
					 vs_identity_column := 'ADOPTION_SUSPENSION_REVISION_ID';
	...
	loop EXIT WHEN vl_months < 0::bigint ;
		...
		-- Calculate Adoption under/over up to previous month - START
		IF vd_current_month_start_dt < vd_previous_month_end_dt THEN
		
			-- Re-calculate Subsidy end date and get Client's DOB & Age
			...
			-- Process Adoption Subsidy payment upto client's 21 birthday - START
			IF vl_client_dob_dt IS NOT NULL
				...
				WHERE TAS.ADOPTION_ID = vl_adoption_id	
					...
					/*AND TAS.SUBSIDY_AGREEMENT_ID = (SELECT MAX(TAS1.SUBSIDY_AGREEMENT_ID)
														FROM tb_ADOPTION_SUBSIDY_AGREEMENT TAS1
															WHERE TAS1.ADOPTION_ID = vl_adoption_id
													  AND TAS1.APPROVAL_STATUS_CD = '3047'
													  AND TAS1.AGREEMENT_START_DT <= vd_current_month_end_dt
													  AND TAS1.AGREEMENT_END_DT >= vd_current_month_start_dt 
													) ;--*/
				...
				IF al_sqlcode < 0 THEN
					 ...
					 vs_identity_column := 'Adoption ID';
					 vs_identity_val := (vl_adoption_id)::character varying;
					--SIGNAL p_sp_error  ;
				END IF ;
				...
				IF vd_current_month_end_dt <= DATE('2009-08-31') THEN -- Monthly Rate
					IF vdc_payment_amount > 0 THEN
						...
						-- VINEET 10/30/2019
						-- SET vdc_gross_amount = vdc_payment_amount;
						...
						SELECT  a.al_linked_pymnt_hdr_id,a.al_reference_payment_detail_id,a.adc_current_balance,a.al_sqlcode,a.as_error FROM SP_GET_CURRENT_BALANCE(vl_adoption_id,
															vd_current_service_start_dt,
															vd_current_service_end_dt,
															vs_room_board,
															vd_current_month_start_dt,
						...
						IF al_sqlcode <> 0 THEN
						   ...
						   IF as_error is NULL OR as_error = '' THEN
							   as_error := 'SP_GET_CURRENT_BALANCE failed - ADOPTION SUBSIDY ADJ (Monthly Rate) NOT CREATED'  ;
						   END IF;
						   --SIGNAL p_sp_error  ;
						END IF ;
						...
						ELSEIF vdc_gross_amount > vdc_current_balance THEN -- Payable
							...
							IF vl_adjust_payment_id = 0 or vl_adjust_payment_id is NULL THEN
								...
								IF al_sqlcode <> 0 THEN
									 as_error := as_error  ;
									IF as_error is NULL OR as_error = '' THEN
										 as_error := 'SP_PAYMENT_HEADER_INSERT failed - ADOPTION SUBSIDY ADJ (Monthly Rate) NOT CREATED'  ;
									END IF;
									--SIGNAL p_sp_error  ;
								END IF ;
							ELSE
							...
							IF al_sqlcode <> 0 THEN
								 as_error := as_error  ;
								IF as_error is NULL OR as_error = '' THEN
									 as_error := 'SP_PAYMENT_DETAIL_INSERT failed - ADOPTION SUBSIDY ADJ (Monthly Rate) NOT CREATED'  ;
								END IF;
								--SIGNAL p_sp_error ;
							END IF ;
							...
							WHERE PAYMENT_ID = vl_payment_id;
							 ...
							 al_sqlcode := SQLCODE;
							IF al_sqlcode <> 0 THEN
								 as_error := 'Error in Updating PAYMENT HEADER ADOPTION SUBSIDY (Monthly Rate) - Gross Amount'  ;
						...
						ELSE -- Receivable
							...
							IF vl_payment_detail_id = 0 OR vl_payment_detail_id is NULL THEN
								 as_error := 'No PAYMENT_DETAIL_ID found for Receivable. (ADOPTION SUBSIDY - Monthly Rate)'  ;
								 vs_identity_column := 'Adoption ID';
								 vs_identity_val := (vl_adoption_id)::character varying;
								--SIGNAL p_sp_error  ;
							END IF ;
						   ...
						   SELECT a.al_sqlcode,a.as_error from SP_OVER_PAYMENT (vl_payment_detail_id,
							...
							IF al_sqlcode <> 0 THEN
							   as_error := as_error  ;
							  IF as_error is NULL OR as_error = '' THEN
								  as_error := 'SP_OVER_PAYMENT failed - ADOPTION SUBSIDY (Monthly Rate)'  ;
							  END IF;
							  --SIGNAL p_sp_error ;
							END IF ;
						END IF;
				...
				ELSE -- > DATE('2009-08-31') - Per Diem Rate
					...
					--FOR CUR_ADOPTION_RATES AS
						...
						ORDER BY TAS.RATE_CREATE_TS; --  DESC; -- (CDM-14135)	
					--DO
					loop
					fetch CUR_ADOPTION_RATES_refcur into CUR_ADOPTION_RATES;
						...
						-- INITIAL VALUES
						 ...
						 vl_provider_id := CUR_ADOPTION_RATES.PROVIDER_ID;
						...
						END IF;	
						--- HOW MUCH WE WERE SUPPOSE TO PAY (Per Diem Rate) - END
					...
					IF al_sqlcode <> 0 THEN
					    as_error := as_error  ;
					   IF as_error is NULL OR as_error = '' THEN
						   as_error := 'SP_GET_CURRENT_BALANCE failed - ADOPTION SUBSIDY ADJ NOT CREATED'  ;
					   END IF;
					  -- SIGNAL p_sp_error  ;
					END IF ;
					...
					ELSEIF vdc_gross_amount > vdc_current_balance THEN -- Payable
						...
						IF vl_adjust_payment_id = 0 or vl_adjust_payment_id is NULL THEN
							...
							IF al_sqlcode <> 0 THEN
								 as_error := as_error  ;
								IF as_error is NULL OR as_error = '' THEN
									 as_error := 'SP_PAYMENT_HEADER_INSERT failed - ADOPTION SUBSIDY ADJ NOT CREATED'  ;
								END IF;
								--SIGNAL p_sp_error  ;
							END IF ;
						ELSE
						...
						IF al_sqlcode <> 0 THEN
							 as_error := as_error  ;
							IF as_error is NULL OR as_error = '' THEN
								 as_error := 'SP_PAYMENT_DETAIL_INSERT failed - ADOPTION SUBSIDY ADJ NOT CREATED'  ;
							END IF;
							--SIGNAL p_sp_error;
						END IF ;
						...
						WHERE PAYMENT_ID = vl_payment_id;
						 ...
						 al_sqlcode := SQLCODE;
						IF al_sqlcode <> 0 THEN
							 as_error := 'Error in Updating PAYMENT HEADER ADOPTION SUBSIDY ADJ - Gross Amount'  ;
							 vs_identity_column := 'Payment ID';
							 vs_identity_val := (vl_payment_id)::character varying;
							--SIGNAL p_sp_error  ;
						END IF ;
					...
					ELSE -- Receivable
						...
						IF vl_payment_detail_id = 0 OR vl_payment_detail_id is NULL THEN
							 as_error := 'No PAYMENT_DETAIL_ID found for Receivable. (ADOPTION SUBSIDY)'  ;
							 vs_identity_column := 'Adoption ID';
							 vs_identity_val := (vl_adoption_id)::character varying;
							--SIGNAL p_sp_error  ;
						END IF ;
					   ...
					   SELECT a.al_sqlcode,a.as_error from  SP_OVER_PAYMENT (	vl_payment_detail_id,
						...
						IF al_sqlcode <> 0 THEN
						   as_error := as_error  ;
						  IF as_error is NULL OR as_error = '' THEN
							  as_error := 'SP_OVER_PAYMENT failed - ADOPTION SUBSIDY'  ;
						  END IF;
						  --SIGNAL p_sp_error ;
						END IF ;
					END IF;
			...
			END IF;			
			-- Process Adoption Subsidy payment upto client's 21 birthday - END
		END IF;
...
--END FOR;	
END LOOP;
close CUR_UNDER_OVER_ADOPTION_refcur;
-- Adoption Subsidy Under/Over main cursor - END

SELECT a.al_sqlcode,a.as_error from SP_SUSBSIDY_RECEIVABLES('A', ad_run_dt) a into al_sqlcode, as_error;
IF al_sqlcode <> 0 THEN
   as_error := as_error  ;
  IF as_error is NULL OR as_error = '' THEN
	  as_error := 'SP_SUSBSIDY_RECEIVABLES failed - ADOPTION SUBSIDY'  ;
  END IF;
  --SIGNAL p_sp_error ;
END IF ;								   
