DROP FUNCTION IF EXISTS cjams.sp_list_payable_approvels(character varying, character varying, character varying, character varying, bigint, bigint, character varying, character varying, character varying, boolean);
DROP FUNCTION IF EXISTS cjams.sp_list_payable_approvels(character varying, character varying, character varying, bigint, bigint, character varying, character varying, character varying, boolean);
CREATE OR REPLACE FUNCTION cjams.sp_list_payable_approvels(v_securityid character varying, v_role character varying, v_status character varying, pagenumber bigint, pagesize bigint, v_sortcolumn character varying DEFAULT NULL::character varying, v_sortorder character varying DEFAULT NULL::character varying, v_searchobj json DEFAULT NULL::json, assigned_pa boolean DEFAULT true)
...
justification_tx character varying, cost_no numeric, voucher character, fiscal_category_cd character, fiscal_category_desc character varying, payee character varying, ads_approval_status_cd character varying, service_log_id integer, actionstatus text, payment_end_dt date, payment_start_dt date, provider_id integer,
remarks text, isapproved integer, request_date date, client_account_id integer, routingstatustypeid integer, reason_tx text, objecttypekey character varying, bmanualrouting boolean, programkey character varying, description_tx character varying, case_id bigint, service_id integer)
 LANGUAGE plpgsql
AS $function$
...
-- 02/20/2024 Sreekanth Marriaknti - CDM-37295 - Added activeflag check to person table to ignore inactive records
-- 02/10/2026 Sandeep Kiran Anugolu - CDM-37295 - Query optimization by removing unnecessary joins and added indexes on tables
...
v_authorization_id := (v_searchObj ->> 'authorization_id'):: bigint;
v_service_log_id := (v_searchObj ->> 'service_log_id')::bigint;
v_fiscal_category_code := v_searchObj ->> 'fiscal_category_code';
v_request_date := (v_searchObj ->> 'request_date')::date;
v_cost_no := (v_searchObj ->> 'cost_no')::numeric;
...
EXISTS (
  SELECT 1
  FROM tb_service_purchase_authorization tss
  WHERE tss.authorization_id = tspa.authorization_id
    AND tss.payment_approval_status_cd = '3047'
) AS ispaymentapproved,
...
,tspa.end_dt,tspa.start_dt,tp.provider_id,case when coalesce(v_status,'') = 'A' then 'Approved'
 when r.activeflag=1 and r.routingstatustypeid!=62  then 'Pending' else (select r.remarks from routing r where r.activeflag=1 and r.objectid=tspa.authorization_id :: character varying and r.eventcode in ('PCAUTH','PCAUTHR') limit 1)  end as remarkss,
...
,tspa.reason_tx,r.objecttypekey,case when r.objecttypekey ='ServiceCase' then false else true end as is_servicecase,tsl.agency_program_area_id
...
AND (v_authorization_id IS NULL OR tspa.authorization_id = v_authorization_id)
AND (v_service_log_id IS NULL OR tspa.service_log_id = v_service_log_id)
AND (
  v_fiscal_category_code IS NULL
  OR concat(tfcm.fiscal_category_desc, tspa.fiscal_category_cd)
     ILIKE '%' || v_fiscal_category_code || '%'
)
AND (v_request_date IS NULL OR tspa.sprvsr_approval_dt = v_request_date)
AND (v_cost_no IS NULL OR tspa.cost_no = v_cost_no)

--GROUP BY
-- tsl.client_id,pname,tss.ispaymentapproved,p.dob,gt.typedescription,ts.service_nm, providernm,tsl.start_dt,
