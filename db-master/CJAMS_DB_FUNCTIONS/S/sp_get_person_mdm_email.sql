Drop function if exists sp_get_person_mdm_email(uuid);

CREATE OR REPLACE FUNCTION cjams.sp_get_person_mdm_email(person_id uuid)
 RETURNS TABLE(personid uuid, "dateOfBirth" character varying, "sourceSystem" character varying, "sourceKey" character varying, "requestId" bigint, "role" character varying, "genderCd" character varying, names json, "emailAddresses" json)
...
-- To get details for person save for email (CIDM-11131)
-------------------------------------------------------------------------------------------
DECLARE 
	vs_person_id uuid;
	...
	RETURN QUERY
		...
		( 
			...
			( 
				...
				FROM personemail  pm                  
				WHERE pm.personid = vs_person_id and pm.activeflag = 1 and pm.personemailtypekey = 'P' order by pm.updatedon desc
			) AS e
		) ::json
		FROM
			person p
