CREATE OR REPLACE FUNCTION cjams.sp_receivable_offset_providers(ad_prev_month_start date, ad_prev_month_end date)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
OPEN cur_providers_refcur FOR
       ...
       WHERE PH.PAYMENT_ID = PS.PAYMENT_ID AND
             ...
             PH.DELETE_SW = 'N' AND
             PS.DELETE_SW = 'N' AND
             F_PRVPCKLST_CAT(PH.PROVIDER_ID,'PLACEMENT') IN ('3049','3274','3302','3794');
...
loop
    ...
    IF vdc_total_gross_amount > 0 THEN
	  ...
	  IF vl_sqlcode <> 0 THEN
	      ...
	      vs_identity_val := (vl_provider_id)::character varying || '/ ' || (vl_payment_header_id)::character varying ;--
	      as_error := as_error  ;--
