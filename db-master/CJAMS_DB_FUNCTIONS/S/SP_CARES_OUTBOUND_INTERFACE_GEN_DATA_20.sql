-- FUNCTION: cjams.sp_cares_outbound_interface_gen_data_20(integer, bigint, character varying, integer, timestamp without time zone)

-- DROP FUNCTION cjams.sp_cares_outbound_interface_gen_data_20(integer, bigint, character varying, integer, timestamp without time zone);
CREATE OR REPLACE FUNCTION cjams.sp_cares_outbound_interface_gen_data_20(vl_client_id integer, vl_other_id bigint, vs_transaction_type_cd character varying, vl_transaction_sequence integer, vd_transaction_ts timestamp without time zone, OUT vs_message character varying, OUT vl_output_sqlcode character varying)
 RETURNS record
 LANGUAGE plpgsql
AS $function$
------------------------------------------------------------------------
...
------------------------------------------------------------------------
    ...
    -- Client Variables
     ...
     CARES_OUT_LIQUID_ASSETS_COL1 CHAR(5);--
     CARES_OUT_LIQUID_ASSETS_COL2 VARCHAR(50);--
     CARES_OUT_LIQUID_ASSETS_COL3 DECIMAL(10,2);--
     CARES_OUT_LIQUID_ASSETS_COL3_CONV VARCHAR(15);--
     CARES_OUT_LIQUID_ASSETS_COL4 VARCHAR(20);--
     CARES_OUT_LIQUID_ASSETS_COL5 VARCHAR(50);--




    DECLARE CURSOR_LIQASSETS  CURSOR FOR
         SELECT /* CASE WHEN personasset.assettypekey = 'BOT' THEN '72'
		             WHEN personasset.assettypekey = 'RT' THEN '82'
					 WHEN personasset.assettypekey = 'VEH' THEN '86'
					 WHEN personasset.assettypekey = 'LI' THEN '77'
					 WHEN personasset.assettypekey = 'RES' THEN '81'
					 WHEN personasset.assettypekey = 'NON' THEN '80'
 */				CASE 
   ...
   EXCEPTION WHEN OTHERS THEN 
    ...
    IF  VS_RECORD_TYPE = '20' THEN
       ...
       WHILE VL_OUTPUT_SQLCODE = '00000'  LOOP
             FETCH CURSOR_LIQASSETS INTO  CARES_OUT_LIQUID_ASSETS_COL1, CARES_OUT_LIQUID_ASSETS_COL2,
                                CARES_OUT_LIQUID_ASSETS_COL3, CARES_OUT_LIQUID_ASSETS_COL4,
                                CARES_OUT_LIQUID_ASSETS_COL5 ;--
	     ...
	     VS_RECORD_SEQUENCE := LTRIM(RTRIM(VL_RECORD_SEQUENCE::VARCHAR));--
	      ...
	      CARES_OUT_LIQUID_ASSETS_COL3_CONV := LTRIM(RTRIM(CARES_OUT_LIQUID_ASSETS_COL3::VARCHAR)) ;--
	
	      CARES_OUT_LIQUID_ASSETS_COL3_CONV := SUBSTRING('00000000000',1,11 - LENGTH(CARES_OUT_LIQUID_ASSETS_COL3_CONV)) || CARES_OUT_LIQUID_ASSETS_COL3_CONV ;	--
		...
		CASE WHEN LENGTH(VS_RECORD_SEQUENCE) = 1 THEN '00'||VS_RECORD_SEQUENCE
		     ...
		     END,
		CARES_OUT_LIQUID_ASSETS_COL1,
		CARES_OUT_LIQUID_ASSETS_COL2,
                CASE WHEN CARES_OUT_LIQUID_ASSETS_COL3_CONV = '' THEN '00000000.00'
                     WHEN LENGTH(CARES_OUT_LIQUID_ASSETS_COL3_CONV) <= 0 THEN '00000000.00'
                     ELSE COALESCE(CARES_OUT_LIQUID_ASSETS_COL3_CONV::VARCHAR,'00000000.00')
                END,  		
		CARES_OUT_LIQUID_ASSETS_COL4,
		CARES_OUT_LIQUID_ASSETS_COL5
	    ;--
