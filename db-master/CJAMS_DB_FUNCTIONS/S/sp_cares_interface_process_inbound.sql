CREATE OR REPLACE FUNCTION cjams.sp_cares_interface_process_inbound(vs_batch_user character varying, OUT vs_message character varying, OUT vl_output_sqlcode character varying, OUT a timestamp without time zone, OUT b timestamp without time zone)
 RETURNS record
 LANGUAGE plpgsql
AS $function$
...
-- Process CARES Inbound Data

-- Revision(s)
-- Hema 12/13/05 - CARES INBOUND GET VALUES FOR THE NEW COLUMN
-- INFO_PROVIDEDBY_TX IN personinsurance(Incident 5411)
-- Mohan    01/16/06   #5955
-- MOhan    02/07/06   #6355
-- 04/24/2007 - #13722 - sandhya - change condition that checks IF cis_client_id exists in tb_client for performance
-- Sandhya - 02.26.2009 - #18364 - Populate Disregard_sw in TB_CLIENT_INCOME only on INSERT with default value 'N'
-- Sandhya - 03.06.2009 - #18364 - Populate Disregard_sw in TB_CLIENT_INCOME only on INSERT with default value 'Y'
-- Vineet Tirodkar - 03.06.2009 - #18364 - Populate notes with default Text.
-- Gayathri Rajkumar - 04.13.2010 - Modified the logic to populate notes all the time
-- Raghu T - 08.24.2012 - Modified Return 1 to Return 0 for Prj-02667
-- 07/06/2020 Vineet Tirodkar - To report Person's Insurance info use personhealthinsurance table (Instead of personinsurance)
-- 12/14/2020 Vineet Tirodkar - Modifications to update statusflag as 0 in caresclientassets & caresclient tables
...
BEGIN       
    ...
    BEGIN    
		...
		LOOP
			...
			BEGIN            
				...
				EXCEPTION WHEN OTHERS THEN
					VS_MESSAGE := 'SELECT  CLIENT_ID FROM TB_CLIENT FAILED FOR CIS_CLIENT_ID:' || VS_CIS_CLIENT_ID   || SQLERRM  ;
					...
					SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
            ...
            IF VL_CLIENT_ID > 0 THEN -- For personhealthinsurance records
				...
				LOOP
					...
					IF VD_MA_ELIG_END_DT < VD_MA_ELIG_START_DT THEN
						BEGIN    
							...
							EXCEPTION WHEN OTHERS THEN
								VS_MESSAGE := 'UPDATE activeflag FAILED FOR  TABLE caresclientma'  || SQLERRM  ;
					...
					IF EXISTS (	SELECT 1 
                                ...
                                WHERE phlt.personid = p.personid
									...
									AND p.activeflag = 1 
								/*
									SELECT 1 FROM personinsurance pi, person p
									WHERE pi.personid = p.personid
										and p.cjamspid = VL_CLIENT_ID
										AND pi.caresauno = VL_AU_NO 
										AND pi.activeflag = 1 
										AND p.activeflag = 1 
								*/			
							  ) THEN
                        BEGIN
							...
							-- effectivedate = VD_MA_ELIG_START_DT,
							-- expirationdate = VD_MA_ELIG_END_DT,
							-- policynumber = VS_MA_ID, 
												
							/*
							UPDATE personinsurance 
								SET caresmatypekey = VS_MA_TYPE,
									medicaidstartdate = VD_MA_ELIG_START_DT,
									medicaidenddate = VD_MA_ELIG_END_DT,
									manumber = VS_MA_ID, 
							...
							*/	
							EXCEPTION WHEN OTHERS THEN
								...
								VS_MESSAGE := 'UPDATE FAILED FOR TABLE personhealthinsurance'  || SQLERRM  ;
								INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
									errorcode, errorsqlcode, errordescription)
								SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                    ...
                    ELSE
                        BEGIN   
							...
							*/
							EXCEPTION WHEN OTHERS THEN
								VL_OUTPUT_SQLCODE:=SQLSTATE;
								VS_MESSAGE := 'INSERT FAILED FOR TABLE personhealthinsurance'  || SQLERRM  ;
                    ...
                    BEGIN
                        UPDATE caresclientma  
							SET statusflag = 0 
						WHERE CURRENT OF MA_INBOUND;
						
                        EXCEPTION WHEN OTHERS THEN
							VL_OUTPUT_SQLCODE  :=  SQLSTATE;
							VS_MESSAGE := 'UPDATE STATUS_SW FAILED FOR  TABLE caresclientma'  || SQLERRM  ;
							INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
								errorcode, errorsqlcode, errordescription)
							SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                ...
                LOOP
					...
					IF VS_EMPLOYER_DELETE_SW = 0 THEN
						IF EXISTS (	SELECT 1 FROM personemployment pe,person p
							...
							IF VD_EMPLOYER_END_DT IS NULL THEN
								BEGIN   
									...
									EXCEPTION WHEN OTHERS THEN
										VL_OUTPUT_SQLCODE  :=  SQLSTATE;
										VS_MESSAGE := 'UPDATE activeflag FAILED FOR  TABLE personemployment '  || SQLERRM  ;
										INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber,
											errorlineno, errorcode, errorsqlcode, errordescription)
										SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                            ...
                            IF VD_EMPLOYER_END_DT IS NOT NULL THEN
								BEGIN          
									...
									EXCEPTION WHEN OTHERS THEN
										VL_OUTPUT_SQLCODE  :=  SQLSTATE;
										VS_MESSAGE := 'UPDATE enddate FAILED FOR  TABLE personemployment '  || SQLERRM  ;
										INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber,
											errorlineno, errorcode, errorsqlcode, errordescription)
										SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                        ...
                        BEGIN        
							UPDATE caresclientemployer 
								SET statusflag = 0 
							WHERE CURRENT OF EMP_INBOUND;
							
							EXCEPTION WHEN OTHERS THEN
								VL_OUTPUT_SQLCODE  :=  SQLSTATE;
								VS_MESSAGE := 'UPDATE STATUS_SW FAILED FOR  TABLE caresclientemployer'  || SQLERRM  ;
								INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
									errorcode, errorsqlcode, errordescription)
								SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                    ...
                    IF EXISTS (	SELECT 1 FROM personemployment 
                        ...
                        BEGIN
							...
							EXCEPTION WHEN OTHERS THEN
								VL_OUTPUT_SQLCODE  :=  SQLSTATE;
								VS_MESSAGE := 'UPDATE FAILED FOR TABLE personemployment'  || SQLERRM  ;
								INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
									errorcode, errorsqlcode, errordescription)
								SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                    ...
                    ELSE
						BEGIN
							...
							EXCEPTION WHEN OTHERS THEN
								VL_OUTPUT_SQLCODE :=  SQLSTATE;
								VS_MESSAGE := 'INSERT FAILED FOR TABLE personemployment'  || SQLERRM  ;
								INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
									errorcode, errorsqlcode, errordescription)
								SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
				...
				LOOP
					...
					IF VS_INCOME_DELETE_SW = 0 THEN
						IF EXISTS ( SELECT * FROM personincome ci,person pp
							...
							IF VD_INCOME_END_DT IS NULL THEN
								...
								BEGIN
									...
									EXCEPTION WHEN OTHERS THEN
										VL_OUTPUT_SQLCODE  :=  SQLSTATE;
										VS_MESSAGE := 'UPDATE activeflag FAILED FOR  TABLE personincome'  || SQLERRM  ;
										INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber,
											errorlineno, errorcode, errorsqlcode, errordescription)
										SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                            ...
                            IF VD_INCOME_END_DT IS NOT NULL THEN
								BEGIN              
									...
									EXCEPTION WHEN OTHERS THEN
										VL_OUTPUT_SQLCODE  :=  SQLSTATE;
										VS_MESSAGE := 'UPDATE enddate FAILED FOR  TABLE personincome'  || SQLERRM  ;
										INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber,
											errorlineno, errorcode, errorsqlcode, errordescription)
										SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                        ...
                        BEGIN       
							...
							EXCEPTION WHEN OTHERS THEN
								VL_OUTPUT_SQLCODE  :=  SQLSTATE;
								VS_MESSAGE := 'UPDATE STATUS_SW FAILED FOR  TABLE caresclientincome'  || SQLERRM  ;
								INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
									errorcode, errorsqlcode, errordescription)
								SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                    ...
                    BEGIN
                        ...
                        EXCEPTION WHEN OTHERS THEN
							VL_OUTPUT_SQLCODE  :=  SQLSTATE;
							VS_MESSAGE := 'SELECT chessieincometypekey FAILED FOR TABLE caresincometype'  || SQLERRM  ;
                    ...
                    BEGIN
						...
						EXCEPTION WHEN OTHERS THEN
							VL_OUTPUT_SQLCODE  :=  SQLSTATE;
							VS_MESSAGE := 'SELECT caresincomeverftypekey FAILED FOR TABLE caresincomeverificationtype'  || SQLERRM  ;
							INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
								errorcode, errorsqlcode, errordescription)
							SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                    ...
                    BEGIN 
						...
						EXCEPTION WHEN OTHERS THEN
							VL_OUTPUT_SQLCODE  :=  SQLSTATE;
							VS_MESSAGE := 'SELECT chessieincomefreqkey FAILED FOR TABLE caresincomefrequencytype'  || SQLERRM  ;
							INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
								errorcode, errorsqlcode, errordescription)
							SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                    ...
                    /* IF  VS_INCOME_TYPE = 'U' THEN
                                SET VS_NOTES_TX = 'This income was imported from CARES. Displayed Start Date is the date CARES entered the income information into their system.' ;
                                IF VL_INCOME_SOURCE_ID = 23 OR VL_INCOME_SOURCE_ID = 22 THEN
                                    SET VS_NOTES_TX = VS_NOTES_TX || ' CARES Income Type = ' || VS_CARES_INCOME_TYPE_DESC ;
                                END IF;
                        ELSE
                               -- #18364 03/09/2009
                                -- SET VS_NOTES_TX = '' ;
                        ...
                        END IF;
                    */
                    --Gayathri Commented the above IF condn to populate Desc all the time
                    VS_NOTES_TX := 'This income was imported from CARES. Displayed Start Date is the date CARES entered the income information into their system.' ;
                    VS_NOTES_TX := VS_NOTES_TX || ' CARES Income Type = ' || VS_CARES_INCOME_TYPE_DESC ;
                    ...
                    IF EXISTS (	SELECT 1 FROM personincome
                        ...
                        BEGIN
							...
							EXCEPTION WHEN OTHERS THEN
								VL_OUTPUT_SQLCODE  :=  SQLSTATE;
								VS_MESSAGE := 'UPDATE FAILED FOR TABLE personincome'  || SQLERRM ;
								INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
									errorcode, errorsqlcode, errordescription)
								SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
					...
					ELSE
						...
						BEGIN
							...
							EXCEPTION WHEN OTHERS THEN
								VL_OUTPUT_SQLCODE :=  SQLSTATE;
								VS_MESSAGE := 'INSERT FAILED FOR TABLE personincome' || SQLERRM ;
								INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
								errorcode, errorsqlcode, errordescription)
								SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                ...
                LOOP
					...
					IF VS_ASSETS_DELETE_SW = 0 THEN
						...
						IF EXISTS (	SELECT 1 
                            ...
                            BEGIN
								...
								EXCEPTION WHEN OTHERS THEN
									VL_OUTPUT_SQLCODE  :=  SQLSTATE;
									VS_MESSAGE := 'UPDATE activeflag FAILED FOR  TABLE personasset' || SQLERRM ;
									INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
										errorcode, errorsqlcode, errordescription)
									SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                    ...
                    BEGIN       
						...
						EXCEPTION WHEN OTHERS THEN
							VL_OUTPUT_SQLCODE  :=  SQLSTATE;
							VS_MESSAGE := 'SELECT CHESSIE_ASSET_VERF_CD FAILED FOR TABLE caresassetverificationtype'  || SQLERRM  ;
							INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
								errorcode, errorsqlcode, errordescription)
							SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                    ...
                    BEGIN
						...
						EXCEPTION WHEN OTHERS THEN
							VL_OUTPUT_SQLCODE  :=  SQLSTATE;
							VS_MESSAGE := 'SELECT CHESSIE_ASSET_TYPE_CD FAILED FOR TABLE TB_CARES_ASSET_TYPE'  || SQLERRM  ;
							INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
								errorcode, errorsqlcode, errordescription)
								SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
					...
					VS_ASSET_NOTES_TX := 'This asset record was imported from CARES. Displayed Purchase Date is the date CARES entered the asset information into their system.' ;
						...
						IF VS_ASSET_TYPE = 'BD' THEN
							BEGIN
								...
								SET assettypekey = VS_ASSET_TYPE_CD,
									...
									locationname = CASE WHEN VS_ASSET_INSTITUTION_NM IS NULL THEN 'CARES-No Location Provided'
														WHEN VS_ASSET_INSTITUTION_NM = '' THEN 'CARES-No Location Provided'
								...
								EXCEPTION WHEN OTHERS THEN
									VL_OUTPUT_SQLCODE  :=  SQLSTATE;
									VS_MESSAGE := 'UPDATE FAILED FOR TABLE personasset'  || SQLERRM  ;
									INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
										errorcode, errorsqlcode, errordescription)
									SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                        ...
                        IF VS_ASSET_TYPE = 'BD' THEN
                            BEGIN
								...
								SELECT gen_random_uuid(),
									...
									VD_ASSET_AMOUNT,
									VS_ASSET_ACCOUNT_NO,
									CASE WHEN VS_ASSET_INSTITUTION_NM IS NULL THEN 'CARES-No Location Provided'
										WHEN VS_ASSET_INSTITUTION_NM = '' THEN 'CARES-No Location Provided'
										ELSE VS_ASSET_INSTITUTION_NM
									END,
								...
								EXCEPTION WHEN OTHERS THEN
									VL_OUTPUT_SQLCODE  :=  SQLSTATE;
									VS_MESSAGE := 'INSERT FAILED FOR TABLE personasset'  || SQLERRM  ;
									INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
										errorcode, errorsqlcode, errordescription)
									SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
                ...
                IF VL_OUTPUT_SQLCODE = '00000' THEN
					...
					BEGIN                 
						...
						EXCEPTION WHEN OTHERS THEN
							VL_OUTPUT_SQLCODE  :=  SQLSTATE;
							VS_MESSAGE := 'UPDATE OF STATUS_SW FAILED FOR  TABLE TB_CARES_CLIENT'  || SQLERRM  ;
							INSERT INTO interfaceserrorlog (interfaceid, currentruntimestamp, batchnumber, errorlineno,
								errorcode, errorsqlcode, errordescription)
							SELECT 'CARES_INBOUND',CURRENT_TIMESTAMP,'',VL_ERROR_ID,'',VL_OUTPUT_SQLCODE,VS_MESSAGE ;
		...
		EXCEPTION WHEN OTHERS THEN
			VS_MESSAGE := 'FAILED TO GENERATE CJAMS DATA'  || SQLERRM ;
