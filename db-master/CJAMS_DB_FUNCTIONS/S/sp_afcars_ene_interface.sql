CREATE OR REPLACE FUNCTION cjams.sp_afcars_ene_interface(ad_start_dt date, ad_end_dt date)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
------------------------------------------------------------------------
...
-- Date : 03/10/2021
-- Description: CJAMS-E&E AFCARS Interface SP to get E&E response for Element 61 IVAFLAG(IVAFLAG) & Element 63 (XIXFLAG)

-- Revision(s)
-- 03/15/2021 Vineet Tirodkar - Modifications to use exitdate column of intakeservreqchildremoval for Child Removal Return Date
...
BEGIN 
	...
	insert into cjams.afcarseneresponsehistory
		(	afcarseneresponseid, afcarsfostercareid, cjamspid, cisclientid, removaldate, returndate, 
			activeflag, insertedon, updatedon, updatedby, insertedby, 
			"extract", ivaflag, xixflag, dataloaddate
		)
	select afcarseneresponseid, afcarsfostercareid, cjamspid, cisclientid, removaldate, returndate, 
		activeflag, insertedon, updatedon, updatedby, insertedby, 
		"extract", ivaflag, xixflag, current_timestamp
	from cjams.afcarseneresponse;
 	
	-- Delete the prior run data
	delete from cjams.afcarseneresponse;
	...
	OPEN CLNT_CUR;
		...
		OPEN REM_CUR;
			...
			vn_clnt_stg_cnt := ( select count(*) 
									from cjams.afcarseneresponse af
			...
			ELSE
				IF vn_clnt_stg_cnt = 0  THEN
					...
					RAISE NOTICE 'INSERT INTO afcarseneresponse (1)';
										
					insert into cjams.afcarseneresponse
						( 	afcarsfostercareid, 
							cjamspid, 
							cisclientid, 
							removaldate, 
				...
				IF vn_clnt_stg_cnt >= 1 AND vn_rem_stg_cnt = 0 AND vn_ret_stg_cnt = 0  THEN
					...
					RAISE NOTICE 'INSERT INTO afcarseneresponse (2)';			
				...
				IF vn_rem_stg_cnt >= 1 AND vd_removal_dt = vd_return_dt_old THEN
					...
					RAISE NOTICE 'UPDATE afcarseneresponse (1)';
										
					update cjams.afcarseneresponse
						set returndate = vd_return_dt,
						updatedon= current_timestamp,
						updatedby = 'cwadmin'
					where cjamspid = vi_client_id
						and afcarseneresponseid 
							= (	select afcarseneresponseid
									from cjams.afcarseneresponse af
								where af.cjamspid = vi_client_id
								order by afcarseneresponseid desc
								limit 1
							  );
				END IF;
			END IF;	
	...
	RAISE NOTICE 'FINAL UPDATE for afcarseneresponse (1)';
        ...
        update cjams.afcarseneresponse
			...
			updatedon= current_timestamp,
			updatedby = 'cwadmin'
        where afcarseneresponseid
			not in	(	SELECT afcarseneresponseid 
							FROM cjams.afcarseneresponse af
	...
	RAISE NOTICE 'FINAL UPDATE for afcarseneresponse >> extract ';
