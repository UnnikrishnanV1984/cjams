CREATE OR REPLACE FUNCTION cjams.sp_afcars_rempck_details(adt_from character varying, adt_to character varying, al_client_id integer, al_removal_id bigint)
...
from intakeservreqchildremoval where  intakeservreqchildremovalid = AL_REMOVAL_ID                      
 ...
 EXCEPTION WHEN OTHERS THEN
                ...
                VL_OUTPUT_SQLCODE :=SQLERRM;
                                VS_MESSAGE := 'SQLSTATE:' || VS_OUTPUT_STATE || '. SELECT  FAILED FOR REMOVALREASONTYPE #152 FOR -->'|| CAST(AL_CLIENT_ID AS VARCHAR) ;--
                                VL_EXCEP_FLAG := 1;--
