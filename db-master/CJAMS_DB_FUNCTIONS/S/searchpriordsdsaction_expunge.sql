DROP FUNCTION IF EXISTS cjams.searchpriordsdsaction_expunge(uuid, character varying, character varying, character varying);
DROP FUNCTION IF EXISTS cjams.searchpriordsdsaction_expunge(uuid, character varying, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION cjams.searchpriordsdsaction_expunge(v_personid uuid, v_mdmid character varying, v_cisclienid character varying, v_userid character varying, v_cjamspid character varying DEFAULT NULL::character varying)
...
-- case when ISRST.intakeserreqstatustypekey in ('Closed') and ATMSR.updatedon is not null then
-- ATMSR.updatedon
...
from person p
where p.personid in (select *  from f_getmaltreatorperson(ia.investigationallegationid,1))) as maltreatorname,
(
...
where ar.person1id = isra.personid and ar.person2id in (select *  from f_getmaltreatorperson(ia.investigationallegationid,1))
...
JOIN expunge.Investigationmaltreatmentactor_expunge ima on ima.maltreatmentid = im.maltreatmentid AND ima.activeflag =1        
JOIN expunge.intakeservicerequestactor_expunge isra on isra.intakeservicerequestactorid = ima.intakeservicerequestactorid
JOIN person p on p.personid = isra.personid
JOIN expunge.investigationallegation_expunge ia ON  ia.investigationmaltreatmentactorid = ima.investigationmaltreatmentactorid
...
select ina.intakeservicerequestactorid from  expunge.intakeservicerequestactor_expunge ina
...
INNER JOIN  expunge.actor_expunge A on A.personid = P.personid  AND A.activeflag = 1
INNER JOIN  expunge.intakeservicerequestactor_expunge ISRA on ISRA.actorid = A.actorid  AND ISRA.activeflag = 1
WHERE ISRA.intakeserviceid = ISR.intakeserviceid and ISRA.intakeservicerequestpersontypekey = 'AM')
as x):: json 
FROM person pr
inner join expunge.intakeservicerequestactor_expunge Actor on Actor.personid = pr.personid
...
from expunge.intakeservicerequestactor_expunge insa
where insa.intakeservicerequestactorid= Actor.intakeservicerequestactorid
and insa.spexpungementflag =1)
join expunge.IntakeServiceRequest_expunge ISR on ISR.intakeserviceid = Actor.intakeserviceid
...
and (select count(1) from expunge.intakeservicerequestactor_expunge insa where insa.intakeservicerequestactorid= Actor.intakeservicerequestactorid
and insa.spexpungementflag =1) = 0 and pr.activeflag = 1 
 ...
 UNION ALL
	...
	-- Intake / Referrals							   
	 SELECT
		staging.intakenumber::character varying,
		staging.intakenumber::character varying AS DANumber,
		''::character varying ,
		null AS dasubtype,
		jsonb_agg(ref_role.value_text) AS roles,
		staging.daterecieved AS DateReceived,
		(staging.jsondata::json-> 'General' ->> 'CreatedDate')::timestamp without time zone as DateCreated,
		null AS DateCompleted,
		coalesce ((CASE 
		  WHEN ISRST.description IS NOT null and ISR.activeflag = 1 then ISRST.description
		  WHEN EXISTS(select 1 FROM routing R where r.routingstatustypeid = 1 AND r.objectid = staging.intakenumber::character varying
		  ...
		  ELSE (select r2.typedescription FROM routing r,routingstatustype r2 
			where r.routingstatustypeid = r2.sequencenumber  AND r.objectid = staging.intakenumber::character varying
			AND r.eventcode = 'INTR' AND r.activeflag = 1 order by r.insertedon desc limit 1)	 
		  end),'In Progress') AS Status,    
		'Intake' AS datype,
		county.countyname AS county,
		(SELECT * FROM getRestrictedCaseStatus(staging.intakenumber::character varying::text, v_userid)) AS restrictStatus,
		null::json,
		null::text,
		null::json,
		( 	SELECT cast(UP.firstname || ' ' || UP.lastname as character varying) as workername 
		FROM routing R 
		INNER JOIN userprofile UP on UP.securityusersid = R.tosecurityusersid
		WHERE R.objectid = staging.intakenumber::character varying::character varying AND R.activeflag =1 limit 1
	),
	(SELECT json_agg(x) as headofhousehlod FROM ( 
        SELECT distinct concat_ws(' ',coalesce(p.firstname,null),coalesce(p.middlename,null),coalesce(p.lastname,null),coalesce(p.suffix,null) ):: character varying as personname
		FROM  expunge.intakeservicerequestactor_expunge ISRA  
		...
		INNER JOIN expunge.actor_expunge A ON A.actorid = ISRA.actorid AND A.activeflag =1 
		WHERE    ISRA.activeflag =1 AND ISRA.isheadofhousehold = true
		AND ISRA.intakenumber = staging.intakenumber::character varying
		ORDER BY 1 ) as x):: json,
	(SELECT json_agg(x) as allegedmaltreator FROM (
		SELECT DISTINCT P.firstname, P.lastname,P.middlename,P.suffix from person P
		INNER JOIN  expunge.actor_expunge A on A.personid = P.personid  AND A.activeflag = 1
		INNER JOIN  expunge.intakeservicerequestactor_expunge ISRA on ISRA.actorid = A.actorid  AND ISRA.activeflag = 1
		WHERE ISRA.intakenumber = staging.intakenumber::character varying and ISRA.intakeservicerequestpersontypekey = 'AM'
		) as x):: json

	FROM person pr
	inner join expunge.intakeservicerequestactor_expunge Actor on Actor.personid = pr.personid and Actor.activeflag = 1
    ...
    and ref_role.ref_key not in (select insa.intakeservicerequestpersontypekey from expunge.intakeservicerequestactor_expunge insa where insa.intakeservicerequestactorid= Actor.intakeservicerequestactorid and insa.spexpungementflag =1)	
	LEFT JOIN expunge.intakeservicerequest_expunge ISR ON ISR.intakenumber = Actor.intakenumber and ISR.teamtypekey = 'CW' and ISR.activeflag=1 
	LEFT JOIN IntakeSerReqStatusType ISRST ON ISR.IntakeSerReqStatusTypeId = ISRST.IntakeSerReqStatusTypeId										   
    JOIN (
		SELECT y.intakenumber, y.daterecieved, y.jsondata, y.status 
		FROM expunge.intakedastaging_expunge y
		JOIN (
			SELECT max(a.id) as id,a.intakenumber, max(a.versionnumber) as versionnumber from expunge.intakedastaging_expunge a where a.activeflag = 1 and a.teamtypekey = 'CW' and lower(a.status)  in ('complete','closed','pending') group by a.intakenumber
		) x ON x.id = y.id 
	) staging ON staging.intakenumber::character varying = Actor.intakenumber
	LEFT JOIN county ON county.countyid::text = (jsondata::json -> 'General' ->> 'countyid')
	
	WHERE 
		pr.personid=v_personid
		and  (select count(1) from expunge.intakeservicerequestactor_expunge insa where insa.intakeservicerequestactorid= Actor.intakeservicerequestactorid and insa.spexpungementflag =1) = 0
		AND Actor.activeflag = 1
	GROUP BY 
	 staging.jsondata,
...
SELECT
	...
	(SELECT 
		CASE 
			...
			ELSE
				'Pending'
		END
	FROM expunge.intakedastatus_expunge ids WHERE ids.intakenumber::character varying = ISR.intakenumber AND ids.activeflag = 1 LIMIT 1)  AS status,  
	'Intake' AS datype,
	county.countyname AS county,
	...
	( 	SELECT cast(UP.firstname || ' ' || UP.lastname as character varying) as workername 
		FROM routing R 
		INNER JOIN userprofile UP on UP.securityusersid = R.tosecurityusersid
		WHERE R.objectid = ISR.intakenumber AND R.activeflag =1 limit 1
	),
	(SELECT json_agg(x) as headofhousehlod FROM ( 
		SELECT distinct concat_ws(' ',coalesce(p.firstname,null),coalesce(p.middlename,null),coalesce(p.lastname,null),coalesce(p.suffix,null) ):: character varying as personname
		FROM  expunge.intakeservicerequestactor_expunge ISRA  
	...
	INNER JOIN referencevalues ref_role ON ref_role.ref_key = actor.intakeservicerequestpersontypekey and ref_role.referencetypeid in (176) and coalesce(ref_role.teamtypekey,'CW') = 'CW' and ref_role.activeflag = 1
    and ref_role.ref_key not in (select insa.intakeservicerequestpersontypekey from expunge.intakeservicerequestactor_expunge insa where insa.intakeservicerequestactorid= Actor.intakeservicerequestactorid and insa.spexpungementflag =1)	
	JOIN expunge.intakeservicerequest_expunge ISR ON ISR.intakenumber = Actor.intakenumber AND ISR.activeflag = 1 AND ISR.intakenumber ILIKE 'cw%' AND ISR.teamtypekey = 'CW'
