CREATE OR REPLACE FUNCTION cjams.sp_ive_alerts_mytasks_count_redet_status(v_loggedusersid character varying, v_loggeduserrole character varying)
 RETURNS TABLE(cjamspid bigint, redet_status integer)
 LANGUAGE plpgsql
AS $function$
------------------------------------------------------------------------------------------------
-- Revisions:
-- CIDM-11090 - Veera - IVE Performance Enhancements- 02/11/2026
-- CIDM-11127 - IVE Slowness issue fix - 02-11-2026
-- CIDM-11158 - Veera Pagination issue fix - 02-26-2026
------------------------------------------------------------------------------------------------
DECLARE 
...
RETURN QUERY 
with AllAlerts as (
 select * from (select pr.cjamspid, (LEAST((date_trunc('month', tce.start_dt) + (DATE_PART('year', now()) - DATE_PART('year', tce.start_dt)) * interval '1 year' - interval '1 day')::date,tfcj.date_agency_lost_legal_responsibility ) + interval '30 days' )::date - now()::date  as redet_status ,row_number () over ( partition by pr.cjamspid,sc.servicecaseid order by pl.alternateid desc)  as rownum, 
    ...
    and LEAST((date_trunc ('month', tce.start_dt ) + ((DATE_PART('YEAR', now()) - DATE_PART('YEAR', tce.start_dt)) * '1 year'::INTERVAL) - interval '1 day')::date,tfcj.date_agency_lost_legal_responsibility)::date >= '10-16-2025'
    ...
    and not exists (select 1 from tb_eligibility_period tep where tep.eligibility_id = tce.eligibility_id and (tep.delete_sw = 'N' or tep.create_ts::DATE >= '10-16-2025') and 
    ...
    UNION ALL
  select * from (select pr.cjamspid , (LEAST(tfcj.date_agency_lost_legal_responsibility, iscr.exitdate)::date + interval '30 days')::date - now()::date AS due_status ,row_number () over ( partition by pr.cjamspid,sc.servicecaseid order by pl.alternateid desc)  as rownum,
(select r.tosecurityusersid from routing r 
      join placement plt1 on pr.personid = plt1.personid
      where r.activeflag=1 AND r.eventcode='PLTR' AND r.routingstatustypeid::text = '70' and r.toroleid in ('IVESP','IVEEA')
      and plt1.activeflag=1 and (plt1.startdatetime >= iscr.removaldate  or (plt1.enddatetime is null and plt1.startdatetime <= iscr.removaldate))
...
and LEAST(tfcj.date_agency_lost_legal_responsibility, iscr.exitdate)::date >= '10-16-2025'
...
union all
select * from (select pr.cjamspid,(LEAST((date_trunc('month', tce.start_dt) + (DATE_PART('year', now()) - DATE_PART('year', tce.start_dt)) * interval '1 year' - interval '1 day')::date, tfcj.date_agency_lost_legal_responsibility) + interval '30 days')::date - now()::date  AS due_status, row_number () over ( partition by pr.cjamspid,sc.servicecaseid order by pl.alternateid desc)  as rownum,
(select r.tosecurityusersid from routing r 
      join placement plt1 on pr.personid = plt1.personid
      where r.activeflag=1 AND r.eventcode='PLTR' AND r.routingstatustypeid::text = '70' and r.toroleid in ('IVESP','IVEEA')
      and plt1.activeflag=1 and (plt1.startdatetime >= iscr.removaldate  or (plt1.enddatetime is null and plt1.startdatetime <= iscr.removaldate))
...
where btrim(tce.eligibility_type_cd) = '2931' AND tce.delete_sw = 'N' and tce.start_dt::date <= (CURRENT_DATE - INTERVAL '1 year')::date
AND (tce.end_dt IS null or tce.end_dt::date > now()) and (iscr.exitdate is null or iscr.exitdate::date > now())
AND LEAST((date_trunc ('month', tce.start_dt ) + ((DATE_PART('YEAR', now()) - DATE_PART('YEAR', tce.start_dt)) * '1 year'::INTERVAL))::date,tfcj.date_agency_lost_legal_responsibility) >= '10-16-2025'
...
union all 
select * from (select pr.cjamspid,(LEAST(tfcj.date_agency_lost_legal_responsibility, iscr.exitdate)::date + interval '30 days')::date - now()::date AS due_status ,
...
and LEAST(tfcj.date_agency_lost_legal_responsibility, iscr.exitdate)::date <= now()::date
and not exists (select 1 from tb_eligibility_period tep where tep.eligibility_id = tce.eligibility_id and (tep.delete_sw = 'N' or tep.create_ts::DATE >= '10-16-2025') and 
((tep.sqnm_sw = 'I' and tep.delete_sw = 'N' and trim(tep.status_cd) = '2914') or (tep.sqnm_sw = concat ('R',(f_age(date_trunc('month', tce.start_dt)::date,iscr.exitdate::date)) + 2) and tep.approvalstatus is not null)))
and not exists (select 1 from tb_foster_care_judicial tfcj where tfcj.client_id = tce.client_id and tfcj.removal_id = tce.removal_id 
...
union all
SELECT * FROM (
    SELECT pr.cjamspid,((LEAST((tce.start_dt + ('180 day'::INTERVAL))::date,tfcj.date_agency_lost_legal_responsibility, iscr.exitdate) + interval '30 day')::date-now()::date) as redet_status,
           row_number() OVER (PARTITION BY pr.cjamspid,  sc.servicecaseid ORDER BY pl.alternateid DESC) AS rownum,
           (select r.tosecurityusersid from routing r 
      join placement plt1 on pr.personid = plt1.personid
      where r.activeflag=1 AND r.eventcode='PLTR' AND r.routingstatustypeid::text = '70' and r.toroleid in ('IVESP','IVEEA')
...
union all
select * from(
select pr.cjamspid,((pr.dob + interval '18 years')::date + interval '30 days')::date - now()::date AS due_status,0 as rownum,
(select r.tosecurityusersid from routing r 
...
and (pr.dob + interval  '18 Year')::date >= '10-16-2025'
and (pr.dob + interval  '18 Year')::date <= now()::date
and not exists (select 1 from tb_eligibility_period tep where tep.eligibility_id = tce.eligibility_id and (tep.delete_sw = 'N' or tep.create_ts::DATE >= '10-16-2025') and 
(tep.sqnm_sw = 'R' and tep.approvalstatus is not null))
)
union all
select * from ( select pr.cjamspid,((pr.dob + interval '18 years')::date + interval '30 days')::date - now()::date AS due_status,0 as rownum,
(select r.tosecurityusersid from routing r 
...
SELECT A.cjamspid,A.redet_status FROM AllAlerts A where 
