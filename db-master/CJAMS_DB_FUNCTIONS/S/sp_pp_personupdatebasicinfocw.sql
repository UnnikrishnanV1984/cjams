CREATE OR REPLACE FUNCTION cjams.sp_pp_personupdatebasicinfocw(v_personid uuid, persondetails json, v_intakeserviceid uuid, v_securityuserid character varying)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
...
insert
	...
	IF ( (v_person ->> 'personroleid')::uuid IS NOT NULL ) THEN 	
		...
		update personroletype set activeflag = 0 , updatedon = now(), updatedby = v_securityuserid where personroleid = (v_person ->> 'personroleid')::uuid;
		update actor set activeflag=0, updatedon = now(), updatedby = v_securityuserid  where personid= v_personid and (intakeserviceid = v_intakeserviceid or intakenumber = v_intakenumber);
	    update intakeservicerequestactor set activeflag=0, updatedon = now(), updatedby = v_securityuserid  where personid = v_personid and (intakeserviceid = v_intakeserviceid or intakenumber = v_intakenumber);
