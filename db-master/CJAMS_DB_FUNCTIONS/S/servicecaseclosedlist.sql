DROP FUNCTION if exists servicecaseclosedlist( character varying, character varying, integer, integer,character varying);


CREATE OR REPLACE FUNCTION servicecaseclosedlist(v_usersid character varying, v_status character varying, v_page integer, v_limit integer, servicecaseno character varying)
 RETURNS TABLE(totalcount bigint, servicecaseid uuid, servicecasenumber character varying, caseheadname character varying, startdate timestamp without time zone, statustypekey character varying, enddate timestamp without time zone, assignedusername text, insertedon timestamp without time zone, program json, subprogram json, casetype character varying, servicerequesttypekey character varying, legalguardian json)
 LANGUAGE plpgsql
AS $function$
...
v_offset  :=  (v_page  -  1)  *  v_limit;
	...
	RETURN  query
        ...
        SELECT 
			...
			sc.caseheadname, 
			sc.startdate, 
			case when lower(v_status) ='closed' then 'Closed' else sc.statustypekey end, 			
			sc.enddate,
        ...
        FROM ( SELECT r.tosecurityusersid ,sc.* FROM servicecase sc
					inner JOIN routing r ON r.objectid = sc.servicecaseid ::character varying AND r.activeflag =1  
						AND r.routingstatustypeid in (4)
						AND  r.eventcode = 'SRVC'	
						AND   r.fromsecurityusersid = v_usersid 
						inner join servicecasedisposition scd on scd.servicecaseid=sc.servicecaseid
						
					WHERE scd.intakeserreqstatustypekey = 'Closed'
