Drop function if exists cjams.sp_foster_care_private_final();

CREATE OR REPLACE FUNCTION cjams.sp_foster_care_private_final()
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
...
OPEN cur_final_detail_refcur FOR
	...
	--cur2 CURSOR WITH HOLD FOR
        SELECT P.PLACEMENT_ID,
               P.ENTRY_DT,
               (CASE WHEN f_age(C.dob::DATE,vd_previous_month_end_dt) = 21 AND ( (C.dob::DATE + INTERVAL '21 year') < P.EXIT_DT OR P.EXIT_DT is NULL) THEN
	...
	--	vs_validation_status :='1750'; --temporarily added the validation code. need to do the validation module also. //here
        IF vs_validation_status = '1750'  THEN  -- PLACEMENT VALIDATED
			...
			SELECT  SP_ADOPTION_SUBSIDY_CHECK(cur_final_detail.CLIENT_ID,
													cur_final_detail.ENTRY_DT::date) INTO
													VS_RESULT;--
 ...
 RAISE NOTICE '%  as_error', as_error;
	        ...
	        ELSE
				SELECT a.vl_output_sqlcode,a.vs_message from SP_PAYMENT_EXECEPTION ('FC_PRIVATE_FINAL',
										...
										'Client is receiving Adoption Subsidy ' || '' || (cur_final_detail.CLIENT_ID::character varying) ) a INTO
										VL_SQLCODE,
										VS_MESSAGE;--
				...
				IF VL_SQLCODE <> 0 THEN
					 ...
					 vs_identity_val := (cur_final_detail.CLIENT_ID)::character varying;--
	...
	RAISE NOTICE '% vl_checklist_cnt>>>>>>>',vl_checklist_cnt;
		...
		IF al_sqlcode <> 0  THEN
			 ...
			 vs_identity_val := (vl_payment_to_provider_id)::character varying;--
			--SIGNAL p_sp_error  ;--
		END IF ;	--
