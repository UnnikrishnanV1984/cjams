CREATE OR REPLACE FUNCTION cjams.sp_cares_outbound_interface_gen_data_50(vl_client_id integer, vl_other_id bigint, vs_transaction_type_cd character varying, vl_transaction_sequence integer, vd_transaction_ts timestamp without time zone, OUT vs_message character varying, OUT vl_output_sqlcode character varying)
 RETURNS record
 LANGUAGE plpgsql
AS $function$
------------------------------------------------------------------------
...
-- Mofidications are comented under the tag #27062019
-- Revision(s)
-- 03/26/2020 Vineet Tirodkar - To fix Production issue - incomesourcetypekey value is blank 
...
SELECT  
	CASE WHEN personincome.incomesourcetypekey = 'CS' THEN '1'
		...
		WHEN personincome.incomesourcetypekey in  ('SSR')THEN '8'
		...
		WHEN personincome.incomesourcetypekey = 'SSI' THEN '26'
		WHEN personincome.incomesourcetypekey = 'SSB' THEN '27'
		WHEN personincome.incomesourcetypekey = 'SSD' THEN '28'
...
BEGIN    
	...
	IF  VS_RECORD_TYPE = '50' THEN
		...
		WHILE VL_OUTPUT_SQLCODE = '00000'  LOOP
			FETCH CURSOR_INCOME INTO CARES_OUT_INCOME_COL1, 
									CARES_OUT_INCOME_COL2, 
									CARES_OUT_INCOME_COL3,
									CARES_OUT_INCOME_COL4, 
									CARES_OUT_INCOME_COL5 ;
