-- CJAMS-64980: Child Account Other Ancillary Balance Issue
/*
-- Issue Description: 
   
   User request to delete the child account incorrect transactions:
   
	Client ID: 4215510 (Nyla Moore)
	Child Account ID: 1032884 - Conserved
	Transactions: 
	1646577	Adjustments	Error Correction			2025-12-01	2025-12-31	612.00
	1646544	Adjustments	Error Correction			2025-12-01	2025-12-31	612.00
	1646543	Adjustments	Error Correction			2025-12-01	2025-12-31	612.00
	1646477	Receipts	Other (Ancillary Payments)	2025-12-01	2025-12-31	612.00
   
	Root cause: User incorrectly created "Other (Ancillary Payments)" transaction instead of "SSA" and 3 adjustments to fix that   
	Fix Provided: Datafix has been promoted to delete the 4 incorrect transactions.
...
update cjams.tb_account_transaction 	
	set delete_sw = 'Y',
		update_user_id = 'CJAMS-64980',
		update_ts = now()
where transaction_id in (1646477, 1646543, 1646544, 1646577)
	and delete_sw = 'N' ;
...
set obligated_for_anc = 
		...
		where spa.delete_sw  = 'N'
			and spa.authorization_id 
				in (	select tr.authorization_id 
						...
						where tr.client_account_id = 1032884
							and tr.delete_sw = 'N'
	...
	update_ts = now(),
	update_user_id = 'CJAMS-64980'
where ta.client_account_id = 1032884
and ta.delete_sw = 'N' ;
