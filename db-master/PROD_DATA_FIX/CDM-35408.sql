/*
 * CDM-35408 - Delete transactions 1227308 and 1227374 - CJAMS ID 200782617
 * golda.gaskin@maryland.gov
 * remove this two transaction IDs with detail information as below;
 * Client ID# : 200782617
 * Client Name : Serayah Matthew
 * Transaction ID# : 1227308
 * Transaction ID# : 1227374
 * make sure to update the Child Account Balance Amount too
 * 
 */

select client_account_id, transaction_id, transaction_amount_no, benefit_start_dt, benefit_end_dt, notes_tx,
	update_ts, update_user_id, delete_sw 
from cjams.tb_account_transaction 
where transaction_id in (1227374, 1227308)
	and delete_sw  = 'N' ;

update cjams.tb_account_transaction 	
	set delete_sw = 'Y',
		update_user_id = 'CDM-35408',
		update_ts = now()
where transaction_id in (1227374, 1227308)
	and delete_sw  = 'N' ;
...
select routingid, eventcode, routingstatustypeid, remarks, activeflag, updatedby, updatedon  
	from routing
where objectid  in ('1227374', '1227308')
...
set obligated_for_anc = 
		...
		where spa.delete_sw  = 'N'
			and spa.authorization_id 
				in (	select tr.authorization_id 
						...
						where tr.client_account_id = 1021349
							and tr.delete_sw = 'N'
	...
	update_ts = now(),
	update_user_id = 'CDM-35408'
where ta.client_account_id = 1021349
and ta.delete_sw = 'N' ;
