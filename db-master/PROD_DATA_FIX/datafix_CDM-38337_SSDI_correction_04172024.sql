-- CDM-38337 - SSDI posting
/*
-- Issue Description: 
   User Request to delete all wrong SSDI Receipts & Adjustment (Error Correction) transactions
   
-- Client ID: 200178055	(ATHENA	ANASTASIA EFANTIS) - 2615020a-6fc2-4e6d-be7c-e8d5c053dbcd
-- Conserved Account: 
-- Conserved Client Account ID: 1030300 
-- Commigled Account ID: N/A
   
-- Category/ Module: Child Accounts (Finance Management) 
-- Root cause: User Error, the Adjustment (Error Correction) was done as a one transaction instead of creating debit adjustment for each SSDI receipt.
-- Fix provided: Datafix has been promoted to delete all wrong SSDI Receipts & Adjustment (Error Correction) transactions. (CDM-38337)
-- Pull request# N/A
...
-- Delete 
-- 589  Receipts - 5487	SSDI --> $ 26364.00
...
where transaction_id in 
		(	1359335, 1359368, 1359401, 1359434, 1359467, 1359468, 1359501, 1359534, 1359567, 1359568, 1359601,
			1359602, 1359635, 1359668, 1359669, 1359702, 1359703, 1359704, 1359737, 1359738, 1359771	
		)	
	and btrim(transaction_type_cd) = '589'
	and btrim(transaction_source_cd) = '5487'	
	and delete_sw = 'N' ;
	
update cjams.tb_account_transaction 	
	set delete_sw = 'Y',
		update_user_id = 'CDM-38337',
		update_ts = now()
where transaction_id in 
		(	1359335, 1359368, 1359401, 1359434, 1359467, 1359468, 1359501, 1359534, 1359567, 1359568, 1359601,
			1359602, 1359635, 1359668, 1359669, 1359702, 1359703, 1359704, 1359737, 1359738, 1359771	
...
-- 588  Adjustments -  5472	Bank Service Charges -> $ 26364.00
select transaction_id, transaction_amount_no, benefit_start_dt, benefit_end_dt, notes_tx,
	update_ts, update_user_id, delete_sw 
from cjams.tb_account_transaction 
where transaction_id = 1371998
	and btrim(transaction_type_cd) = '588'
	and btrim(transaction_source_cd) = '5472'	
	and delete_sw = 'N' ;
...
set obligated_for_anc = 
		...
		where spa.delete_sw  = 'N'
			and spa.authorization_id 
				in (	select tr.authorization_id 
						...
						where tr.client_account_id = 1030300
							and tr.delete_sw = 'N'
	...
	update_ts = now(),
	update_user_id = 'CDM-38337'
where ta.client_account_id = 1030300
and ta.delete_sw = 'N' ;
