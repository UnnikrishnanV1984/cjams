-- CJAMS-64843 Duplicated funds stuck in Obligation
/*
-- Issue Description: 
   
   Data fix to remove the duplicate Ancillary Goods/Services Obligation transactions on 2 child accounts.
   
	Client PID#: 3453483 (Zyrihanna Thompson)
	Child Account Number: 4945939130 - Conserved - 1017933
	Transaction ID: 1615739 & 1615784
    Auth ID: 4059713
	
	Client PID#: 3759781 (Tyshawn Nicholson-Hearn)
	Child Account Number: 4941346546 - Conserved - 1025064
	Transaction ID: 1615777 & 1615783
	Auth ID: 4059645

	Root cause: Unknown at this point in time, not able to replicate in lower environment.
	Fix Provided: Datafix has been promoted to delete the duplicate Ancillary Goods/Services Obligation transactions.
	Reason why no related code fix: TBD, Not able to replicate in lower environment.

*/

-- Delete Duplicate 
-- '5530' -- Ancillary Goods/Services Obligation and '5474' -- Obligated
update cjams.tb_account_transaction 	
	set delete_sw = 'Y',
		update_user_id = 'CJAMS-64843',
		update_ts = now()
where transaction_id in (1615783, 1615784)
	and btrim(transaction_type_cd) = '5530' -- Ancillary Goods/Services Obligation
	and btrim(transaction_source_cd) = '5474' -- Obligated
	and delete_sw = 'N' ;
...
set obligated_for_anc = 
		...
		where spa.delete_sw  = 'N'
			and spa.authorization_id 
				in (	select tr.authorization_id 
						...
						where tr.client_account_id = 1017933
							and tr.delete_sw = 'N'
	...
	update_ts = now(),
	update_user_id = 'CJAMS-64843'
where ta.client_account_id = 1017933
and ta.delete_sw = 'N' ;
...
set obligated_for_anc = 
	 coalesce(( select sum(spa.cost_no) 
		...
		where spa.delete_sw  = 'N'
			and spa.authorization_id 
				in (	select tr.authorization_id 
							from tb_account_transaction tr
						where tr.client_account_id = 1025064
							and tr.delete_sw = 'N'
							and tr.authorization_id is not null
							and (select count(*)
									from tb_payment_header ph
	...
	update_ts = now(),
	update_user_id = 'CJAMS-64843'
where ta.client_account_id = 1025064
and ta.delete_sw = 'N' ;
