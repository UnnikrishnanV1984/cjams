-- CDM-35816 - Error Correction
/*
-- Issue Description: 
    Data fix to update the Negative Obligated amount for Drezden Moore CID: 3132330 to correct Value
   
-- Client ID: 3132330 (DREZDEN SHAINE MOORE) - 7a6359cf-84b9-4560-97b1-8b286fee9153
   
-- Category/ Module: Child Accounts (Finance Management) 
-- Root cause: Data issue obligated amount was having -ve value due to some old transaction. We will monitor to see if  we are getting any similar issue again to do the further analysis.
-- Fix provided: Datafix has been promoted to update the requested Child Obliged amount transaction and update the balance accordingly.
-- Pull request# N/A
...
set obligated_for_anc = 
		...
		where spa.delete_sw  = 'N'
			and spa.authorization_id 
				in (	select tr.authorization_id 
						...
						where tr.client_account_id = 1017541
							and tr.delete_sw = 'N'
	...
	update_ts = now(),
	update_user_id = 'CDM-35816'
where ta.client_account_id = 1017541
and ta.delete_sw = 'N' ;
