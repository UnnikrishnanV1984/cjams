...
-- Issue Description: 
   ...
   User request to delete the child account incorrect transactions:
   
	Client ID:  3707362 (ZY'AIRE O TROTTER)
    Root cause: The issue occurred because the original survivor benefit deposit was recorded in CJAMS 
    before SSA’s letter clarified the correct allocation. The system split the deposit across multiple months 
    and does not correctly reallocate retroactive adjustments, 
    resulting in duplicate deposits and posting adjustments to Other Ancillary instead of Cost of Care
	Fix Provided: Datafix has been promoted to  remove all child account transactions .
...
update cjams.tb_account_transaction 	
	set delete_sw = 'Y',
		update_user_id = 'CJAMS-67382',
		update_ts = now()
where transaction_id in (1682087, 1682088, 1684299, 1684300)
	and delete_sw = 'N' ;
...
set obligated_for_anc = 
		...
		where spa.delete_sw  = 'N'
			and spa.authorization_id 
				in (	select tr.authorization_id 
						...
						where tr.client_account_id = 1054743
							and tr.delete_sw = 'N'
	...
	update_ts = now(),
	update_user_id = 'CJAMS-67382'
where ta.client_account_id = 1054743
and ta.delete_sw = 'N' ;
