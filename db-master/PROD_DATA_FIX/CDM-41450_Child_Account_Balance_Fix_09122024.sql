-- CDM-41450 - Child Account Balance Incorrect
/*
-- Issue Description: 
	To fix Child Account Balance
   
-- Client ID: 4239508 (AMBER M SCRUGGS) - 78906269-e195-4913-9ace-54a069a7ba98 
-- Foster Care Youth Saving Account ID: 1016077 (F1020934) - $3521.88
-- Comm Acount ID: 1000403

-- Category/ Module: Child Accounts (Finance Management) 
-- Root cause: TBD, looks like the Child Account Balance was not updated the balance correctly after the Adjustments / Bank Service Charges Debit Transaction.
...
-- To re-calcualte update the Child Account balance (CDM-41450)
...
set obligated_for_anc = 
		...
		where spa.delete_sw  = 'N'
			and spa.authorization_id 
				in (	select tr.authorization_id 
						...
						where tr.client_account_id = 1016077
							and tr.delete_sw = 'N'
	...
	update_ts = now(),
	update_user_id = 'CDM-41450'
where ta.client_account_id = 1016077
and ta.delete_sw = 'N' ;
