/*Placement sequence*/
DROP SEQUENCE  IF EXISTS sequence_placement CASCADE; 
CREATE SEQUENCE sequence_placement
    INCREMENT 1
    START 1000001
    MINVALUE 1
...
ALTER  TABLE placement ALTER COLUMN alternateid  SET DEFAULT nextval('sequence_placement'::regclass);
UPDATE placement SET alternateid =nextval('sequence_placement'::regclass) ;

/*Renaming table*/
ALTER TABLE tb_placement RENAME TO tb_placement_delete ;
ALTER TABLE tb_placement_revision RENAME TO tb_placement_revision_old ;



DROP SEQUENCE  IF EXISTS sequence_placementrevision CASCADE; 
CREATE SEQUENCE sequence_placementrevision
    INCREMENT 1
    START 1000001
    MINVALUE 1
    MAXVALUE 9223372036854775807
...
ALTER TABLE cjams.placementrevision ADD COLUMN alternateid bigint;

ALTER  TABLE placementrevision ALTER COLUMN alternateid  SET DEFAULT nextval('sequence_placementrevision'::regclass);
UPDATE placementrevision SET alternateid =nextval('sequence_placementrevision'::regclass) ;

alter table placementrevision alter column isoriginal TYPE character(1);
alter table placement alter column overunderflag TYPE character(1);

ALTER TABLE cjams.tb_placement_auto_validation_log ADD COLUMN IF NOT EXISTS activeflag int4 NULL DEFAULT 1;



/*tb_placement View */
DROP VIEW IF EXISTS tb_placement;

-- View: cjams.tb_placement

-- DROP VIEW cjams.tb_placement;


 CREATE OR REPLACE VIEW cjams.tb_placement
AS SELECT pl.alternateid AS placement_id,
